...
- **慢速段的 turn 是一辆停着的车。** 按上述形态，一个 agent 最长可占用其 turn 约 27 分钟，子任务兄弟与 gather 都为其保持打开——进度卡停在重试播报上，盯着聊天看的用户经历的是一段长静默。替代失败形态（turn 死亡+父级唤醒）可见且有界；慢速段用「可见的失败」换「安静的耐心」，这笔交易只在窗口真的会关闭时才划算。
- **认证类误配若被误分类就不再快速失败。** 慢速段继承 `retryableCodes`，保护力度与该闸门完全一致；若 provider 把凭证失败报成限流码，会烧完全部慢速预算才浮出。快速段既有的分类是唯一事实源，本 note 不改它。
- **拆除路径下的计时器所有权。** 慢速等待跨分钟，会途经今天只会遇到 <30 秒等待的 stall 看门狗、引擎停与 disposal 路径；实施变更必须证明取消能在等待中结算、不留迟到的多余尝试（由上面的中止判据覆盖）。
- **设计有意放弃的：** 本失败类不再有「死 turn 靠父级唤醒重进上下文」——会话从未离开运行集，也就没有带上下文的 idle 态可恢复；provider 键并发信号量（根因修法）在事件复发前保持无主。
