...
- **fork-at 是一次 seeded create。** `prepareForkAtSession` 仍经 `locateForkCut` 截断，但把前缀暂存 adapter 内 map 而非持久化副本；`__forkat__<newID>` 哨兵消费它执行 `agents.create({ sessionId, seed, meta: { cwd, parentSession, seedLength } })`。`seedLength` 按 [seed-boundary note](../../archived/testing/2026-06-22-fork-child-replay-seed-boundary.md) 保持显式。prepare 与 start 之间 daemon 重启会丢暂存 seed；哨兵随即按无源 fork 同款降级为全新会话并 `console.warn`。
