...
操作用户取会话的 spawn user（`session.getSpawnUserID()`）；无活跃会话的 chat 解析为 `''`，admin 判定按非 admin 处理——fail closed。卡片按钮路径由 `handleCardAction` 把 `msg.userID` 传入 `executeCardAction`，卡片动作因此与其余入口同为 owner-or-admin 门；测试构造的无身份调用退化为纯归属判定（fail closed）。
...
信任线现在统一：一切执行 shell 或改变 job 执行内容的行为需要 admin；一切管理既有 job 的行为需要归属或 admin。未来任何新 cron 入口必须走 `cronJobActionAllowed`，不许直连 `scheduler.store()`。卡片路最初落地时没带用户 id（纯归属，admin 也拒），同日跟进批次补上了 `msg.userID` 透传；没有操作者 id 的卡片回调仍 fail closed 到归属判定。
