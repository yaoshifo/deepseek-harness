...
The /ps acknowledgement became a three-state reaction on the triggering message, and the two dead knobs are gone. **Superseded 2026-09-18:** the retraction half is gone — the marks are additive, the pickup is never retired, and every platform takes one path ([additive-marks note](2026-09-18-feishu-bridge-ps-additive-reaction-marks.md)); the claim anchor below, the emoji keys, and the knob removal stand.
...
- **Accumulate Get and DONE without retraction.** Rejected at the time: a lingering Get reads as "still queued" forever; the monitor flow's remove-and-swap is the established pattern — reversed by user ruling on 2026-09-18 and shipped in [the additive-marks note](2026-09-18-feishu-bridge-ps-additive-reaction-marks.md).
...
- Accepted races, documented: a claim event drained in a stall-retry channel swap can leave a Get on an already-claimed steer (under-reports, never lies); a daemon restart loses in-memory records (stale Get). The stop/cleanup settlements cover every engine-initiated teardown path. Since 2026-09-18 the first residue is by design rather than a defect — an unsettled pickup keeps its Get, with no outcome mark ([additive-marks note](2026-09-18-feishu-bridge-ps-additive-reaction-marks.md)).
