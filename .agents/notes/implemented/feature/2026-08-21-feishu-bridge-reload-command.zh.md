...
`src/engine/reload-commands.ts` 新增 TS 原生 `/reload [--skip-build]` 命令（非 Go D 类裁剪 `/restart` 的移植）。handler 先回复「已启动」——`--skip-build` 数秒内就会重启 daemon，spawn 之后再发回复永远到不了——随后以 `spawn('sh', [reload.sh, ...], { detached: true, stdio: ['ignore', logFd, logFd] })` + `unref()` 运行脚本本体。setsid 脱离是承重设计：脚本亲手造成 daemon 之死、又必须活过这次死亡，所以它在动手之前先离开 daemon 的进程组。脚本路径经 `import.meta.url` 自定位（相对包根；live profile 把包符号链接进 repo，daemon 解析到的就是 repo checkout 里的脚本）。
