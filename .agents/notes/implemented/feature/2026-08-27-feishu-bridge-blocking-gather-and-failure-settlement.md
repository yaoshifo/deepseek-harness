...
- Upstream tracking rides the seam, not the tool: runtime-level upgrades in `packages/subagent` flow in with `dsh-sync-upstream` and reach the bridge through `asContinuableDelegator` / `subagent/end` / the session-event projection with zero bridge changes; an interface refactor surfaces loudly at typecheck in the sync PR. The one manual habit per sync: when the diff touches `packages/subagent/tool-*`, review whether a tool-layer improvement is worth porting into `feishu_bridge_subtask`'s contract. Generic seam improvements (the `diagnostic` field) are push-upstream candidates — each one shrinks the divergence surface.
