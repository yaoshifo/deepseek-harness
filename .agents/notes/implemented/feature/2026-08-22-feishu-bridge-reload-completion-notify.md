...
- Different pid, fresh (15-minute TTL covering the build-plus-restart window) — look up the recorded engine by name, then that engine's platform by name, send the `reload_completed` notice through it, delete the marker. The handler ran with `p ∈ engine.platforms` and that bot is in the chat, so the two-level lookup reproduces the exact sender of the "started" reply; `Platform.name()` alone is not an identity because untagged deployments name every platform `'feishu'` (the 230002 lesson).
- Stale, unknown engine or platform (project removed or renamed mid-reload), corrupt JSON, or a failed send (e.g. the /reload message withdrawn) — warn and delete; every consumed path deletes, so one daemon start yields at most one notice.
...
**Per-engine invocation of the notification check.** Engines start concurrently; several projects could share the default platform name `feishu`, so per-engine checks could double-send. One post-`Promise.all` invocation is race-free.

**Trying every same-named platform until one succeeds.** Rejected after the 230002 incident: eight doomed API calls per reload, log noise, and error-code guessing where the marker can simply record the engine name — project names are unique in the config.
...
`tests/engine/reload-commands.spec.ts` (27 cases total, red first): cmdReload writes the marker before the spawn (pid/engine/platform/replyCtx/at) and a non-zero exit clears it alongside the failure reply; `completePendingReload` sends through the matching engine's platform and clears, sends through the *recorded* engine's platform when platform names collide across engines (the 230002 incident shape: two engines, both platforms named 'feishu', the wrong engine listed first), keeps the marker on a pid match (HMR), drops stale (TTL boundary), drops on engine or platform mismatch, is a no-op without a marker, drops corrupt JSON, and still clears after a failed send. Full package suite 2087 green; `tsc` clean; no snapshot — the chat notification cannot enter the record-replay harness (same documented gap as the /reload command itself). Real-device smoke on the dev server: `/reload` in chat receives the completion notice as a reply to the command message; the marker file appears then disappears.
