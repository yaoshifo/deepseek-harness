...
### Model visibility of the whitelist

When `envFile` is configured and a system-prompt service is composed, the executor registers one boot-fixed section (`bash:env-file`) stating these entries are per-command trusted injection whose absence from the daemon process environment is expected, and that values are never printed; the live key names ride in a `DSH_ENVFILE_KEYS` marker rebuilt at each spawn. Motivation: a 2026-08-31 dida-group session spent ten tool calls archaeologizing the injection mechanism ("var present in tool-shell, absent from daemon env") before first use — the announcement answers provenance up front. Rejected alternatives: a `ShellExecutor` seam method consumed by tool-bash's description (cross-package surface for one sentence); a first-touch result note keyed by session (pay-per-use, but runtime state, per-command matching, and result-text mutation for a cost the static section amortizes to near zero through prefix caching).
