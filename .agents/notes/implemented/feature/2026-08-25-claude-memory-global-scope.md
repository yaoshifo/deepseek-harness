# Agent Note: Claude-memory global scope — cross-project memory as an opt-in second directory
...
**Opt-in by a config key carrying its own budget.** `global: { maxIndexBytes, maxIndexLines? }` enables the scope; absent or empty, every model-visible surface stays byte-identical to a global-less deployment (no appendix, no second injection, no `scope` tool parameter). The budget is deliberately a separate, smaller number — the noise hard cap for globally injected content — and every composition must state it explicitly, consistent with the plugin's existing budget discipline. Schemastery quirks shaped the schema: an absent nested object arrives as `{}`, and nested `required()` fields are enforced even when the outer key is absent, so `apply` treats an empty `global` as disabled and rejects an enabled one without a positive byte budget at load.
