...
**Make delivery configurable.** A deployment switch that made inbox delivery conditional would return the model-facing text to "usually", which is the failure this change exists to remove. Protocol constants and safety invariants stay fixed; this is one of them. What did ship is the all-or-nothing `settlementNotice: 'external'` outlet ([the continuable bridge seam note](2026-08-24-subagent-continuable-bridge-seam.md)) — a deployment asserting it delivers the account itself, which keeps the promise unconditional from the parent's point of view, not a per-run switch.
...
Turn amplification is real for deep or wide trees, and inbox delivery is not tunable by design (the `external` outlet removes delivery entirely rather than shaping it). The step-boundary batch bounds it for simultaneous settlement but not for children that settle apart.
