...
空闲父级得到一个普通的后续轮次。繁忙父级则被 steer 到其最近的 step 边界，因为 `Inbox.claim()` 会在一个边界上整批取走 next-step：四个 child 同时结算时因此只消耗一个 step，而不是四个轮次。采用 steer 而非 inject 是刻意的——驱动运行期间该唤醒是空操作，同时它关闭了「驱动在状态读取与发送之间退出」的那个窗口；否则通知会滞留无人认领，直到别的事件唤醒父级。这是正确性规则而非部署偏好：不做成按次生效的 `Config` 字段。现存的唯一部署级出口是 `SubagentRuntime` 的 `settlementNotice: 'external'`（见 [continuable bridge seam note](2026-08-24-subagent-continuable-bridge-seam.zh.md)）——面向自己掌控父级轮次调度、自行投递结算的部署，整份压制 runtime 的投递；它不触碰这里的调度，inbox 模式的投递保持无条件。
...
**把投递做成可配置。** 让 inbox 投递变成条件式的部署开关会把面向模型的文本重新变回「通常」，而这正是本次改动要消除的失效。协议常量与安全不变量保持固定；这就是其中之一。真正落地的是全有或全无的 `settlementNotice: 'external'` 出口（见 [continuable bridge seam note](2026-08-24-subagent-continuable-bridge-seam.zh.md)）——部署以此断言自己投递结算，从父级视角承诺仍是无条件的，而非按次开关。
...
对于深或宽的树，轮次放大是真实存在的，而且 inbox 投递按设计不可调（`external` 出口是整份移除投递，不是塑形）。step 边界的批量语义只能约束同时结算的情形，无法约束分散结算的 child。
