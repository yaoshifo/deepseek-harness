...
3. Duplicate-tag guard: the suffix is appended only when the label carries none of the message table's standard forms anywhere (`includes`, not `endsWith`). The detection set derives from `messages.ask_recommended_suffix` itself — a single source of truth; a registered subtable cannot override a main-table key, so the static derivation can never drift from the rendered value. Both failure modes of `includes` fall on the safe side: a miss leaves the item without the text signal (highlight remains), never a doubled tag.
