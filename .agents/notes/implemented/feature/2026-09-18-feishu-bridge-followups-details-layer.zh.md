...
- 两张卡都把它收进一个按带细节选项数命名的折叠块、不再行内展示——活卡在表单内，结算快照经 `followupsDetailLines`（`src/engine/ask.ts`）重建同一批行，其 marks 只留白话行——见[收尾卡折叠](2026-09-18-feishu-bridge-followups-card-fold.zh.md)。
...
- 卡面不再只是白话：事实细节在两个面上都只在一次点击之外，与计划卡自己的折叠细节形态一致。
- 本次改动之前登记的卡派发时没有细节行——退化为旧行为，不是故障。
- `details` 在工具 schema 里是选填；reload 后首批卡的填写质量会被观察，升级路径是改为必填（与计划卡 details 的推进方式一致）。
- 折叠行的 `**label** · detail` 格式在两处拼装——活卡在飞书渲染器、结算快照在 `followupsDetailLines`——两个面都由[收尾卡折叠](2026-09-18-feishu-bridge-followups-card-fold.zh.md)的用例钉住。
- 测试：`tests/feishu/card-action.spec.ts`（`sendCard` 与 `replyCard` 两条出口的发送时回读都派发细节）、`tests/feishu/card.spec.ts`（结算卡的折叠面板与其行）、`tests/engine/followups.spec.ts`（`followups details` describe）、`tests/tools/followups-tool.spec.ts`（schema 暴露 `details`、不含 `locator`；转换保留它）、`tests/agent-dsh/adapter-persona.spec.ts`（约定文本更新）。
