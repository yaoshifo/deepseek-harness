...
- Newer cards show the factual detail in grey; the card no longer reads as plain language only — the accepted cost of the user's request (the plan card keeps its own collapsed-details pattern).
...
- The grey wrapper is renderer-side for the live card and marks-side for the settled card; both are pinned (`tests/feishu/card.spec.ts` renders the checker text, `tests/engine/followups.spec.ts` asserts the settled marks and the dispatch text).
- Testing: `tests/feishu/card-action.spec.ts` (the send-time read-back through both the `sendCard` and `replyCard` egresses dispatches the detail), `tests/feishu/card.spec.ts` (grey detail line on the live and settled card faces), `tests/engine/followups.spec.ts` (`followups details` describe), `tests/tools/followups-tool.spec.ts` (the schema exposes `details` and no `locator`; conversion keeps it), `tests/agent-dsh/adapter-persona.spec.ts` (conventions text updated).
