...
- The pin lives on the bridge `Session`'s `inheritedMode` field — a chat-scoped record that `carryChatScopedState` already carried across `/new` resets, so the pin survives session rotation inside the chat. It stays in-memory (never serialized): a daemon restart drops the pin and the chat falls back to the project default, the same trade-off as `pendingMonitorClarification`. The field's prior Go-parity write (`parentEffectiveMode()`, meant to inherit the parent's current mode) stored a constant `''` — `InteractiveState.effectiveMode` has no non-`''` writer — and that dead path was removed in the same change, so the field is now written only by an explicit flag.
