...
Plain sessions carry the conventions section (~1450 chars) plus the TDD default (~360) plus the grill section (~350). Subtask children and chatroom personas get neither — same tier as the skillify offer; dsh web sessions are outside the coverage, the same accepted trade the TDD mandate made.
...
Re-measurement criterion (one week after the reload): sample one-line feature-ask sessions for grill skill loads and an ask card before implementation (both should appear where the baseline showed silent dives), and clearly-specified task sessions for spurious interviews (should be ≈0). The 08-28 scan methodology applies (glob session dirs before `zstdcat`; pre-reload sessions are all baseline).
