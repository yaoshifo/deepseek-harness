...
`src/engine/reload-commands.ts` adds a TS-native `/reload [--skip-build]` command (not a port of Go's D-class-cut `/restart`). The handler replies "started" first — `--skip-build` restarts the daemon within seconds, so a reply issued after the spawn would never arrive — then runs the script itself via `spawn(cmd, [argv...], { detached: true, stdio: ['ignore', logFd, logFd] })` plus `unref()` (`reloadSpawnArgv`). The detach is the load-bearing move: the script causes the daemon's death and must survive it. On macOS a setsid child survives launchd teardown; on Linux the spawn goes through `systemd-run --user --scope`, because setsid alone keeps the child in the daemon unit's cgroup, which the restart's control-group kill sweeps away (see [the cgroup note](../bug-fix/2026-08-22-feishu-bridge-reload-linux-cgroup.md)).
