...
- **Upstream shared packages (additive fork-local, to be proposed upstream once stable)**: `exit_plan_mode` gains an optional `details: string` parameter (description stays policy-free — the mechanism is "implementation-detail annex; capable UIs present it collapsed by default", the layering policy remains deployment-owned prompting). The plan-review intent of `dsh-user-questions` gains optional `layers?: { plain, details }`, matching the type's existing presentation-only semantics. execute normalizes a blank `details` to not-submitted; the review question's `detail` carries both layers concatenated (`plan + '\n\n' + details`, so generic consumers such as web/CLI see the complete plan unchanged), and the intent carries `layers` only when `details` was submitted. An exit that inlines an implementation-details section (实施细节 / 技术细节 / Implementation Details / Implementation Notes heading) in `plan` without submitting `details` is rejected with guidance to move the section into the annex — prompt guidance alone left glm-5.3 inlining the section into `plan` on 3 of 4 sampled exits (2026-09-10 Feishu), so detected mislayering surfaces as a corrective tool error before the review card renders flat.
- **Feishu bridge rendering**: the adapter's `answerPlanReview` passes `layers` from the intent through; the engine threads `layers` down to `sendPlanCard` **unless the agent-written plan file override applies**, building the card as `[markdown(plain), collapsiblePanel(implementation details, expanded:false, markdown(details)), actions(export)]` — `collapsible_panel` is an off-the-shelf schema 2.0 component (in production as the "▸ 详细信息" footer on every completion card). Without `layers` the card stays a single markdown block (old path and old sessions unchanged). The export button, the persisted plan file, and the permission card keep consuming the concatenated text; the PNG render image is the approver-facing overview — with `layers` it renders from the plain layer only (the details annex stays in the card's collapsed panel and the export, and unlayered plans render their full body — nothing is split away), with the render-dedup hash following the render input.
...
- **Prompting**: the agent-conventions "plain-language" plan paragraph maps the layers onto the two arguments (plain layer into `plan`, implementation layer into `details`); the implementation layer's substance includes the execution grouping and parallel/serial schedule (which groups run in parallel, which run in order), giving the plan-mode section's required execution-order marking a named home. The self-check sentence is unchanged.
...
**Making `details` required.** Deferred: required would mechanically guarantee layering, but this shared upstream tool serves every deployment. The 2026-09-10 Feishu sampling (3 of 4 exits inlined details into `plan`) landed the narrower corrective gate instead; required remains the next escalation only if gated exits keep failing (the README records the escalation path).

**Putting the execution grouping in the plain layer (the approver-facing layer).** Rejected (user decision, 2026-09-11): dispatch scheduling belongs to the executor — the approver decides what and the acceptance criteria — and no sampled plan had actually mislayered a grouping; naming it in the plain layer would also reintroduce implementation detail into the approval surface this layering exists to relieve.

## Consequences

- Robustness layers: structure = contract (tool schema), semantics = guidance (prompting), failure mode = a corrective rejection for detected mislayering (inlined details section without the annex), graceful degradation otherwise (no `details` and no inlined section → the card falls back to the single block, zero content loss).
...
- An inlined-section swap (detail written into `plan` under a recognized heading) is caught by the rejection gate before review; prose-shaped detail inside `plan` with no recognizable heading still renders the flat card and joins the existing sampling retest (details fill-rate, plain-layer presence, and the grouping marking inside the details layer).

## Testing

`packages/plan/plan-mode/tests/plan-mode.spec.ts`: detail concatenation and intent shape across the with/without/blank `details` paths (the exact assertion at :1069 stays green) plus the two-part presentCall; the rejection gate covers the inlined-section rejection (error text, zero reviewer asks), the details-submitted pass, and the plain `## Details` non-match. `packages/acp/feishu-bridge/tests/agent-dsh/adapter.spec.ts`: `intent.layers` pass-through and unchanged absence. `tests/engine/engine-m3-plan.spec.ts`: layered card structure (`expanded:false`, panel title, export button), no-`layers` regression guard, per-layer truncation. `tests/agent-dsh/adapter-persona.spec.ts`: verbatim pin of the two-argument wording.
