...
- An inlined-section swap (detail written into `plan` under a recognized heading) is caught by the rejection gate before review; prose-shaped detail inside `plan` with no recognizable heading still renders the flat card and joins the existing sampling retest (details fill-rate plus plain-layer presence).
