...
- **Hand-rolled squarify** (`squarify`, ~80 lines, the Bruls algorithm): order-preserving (columns left→right, stacks top→bottom, reading order ≈ assembly order), areas exactly ∝ tokens, greedy row packing keeps aspect ratios square. No d3/echarts: the artifact must open offline self-contained, and the algorithm is smaller than the dependency wiring.
