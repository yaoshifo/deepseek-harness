...
**Part-design red line:** connectors must be structural elements — explicit `<b></b>` arrow elements in `.flow-v`, a real `<i class="stem">` plus the satellite container's `border-top` bus in `.hub`, pairwise circle-to-circle segments in `.stages` — never negative-margin absolute pseudo-element line assembly whose coordinates are guessed. Assembly is verified by browser geometry assertions (Playwright `getBoundingClientRect`: stem ends flush with bus and center, bus spans every tick, arrows centered and adjacent to both nodes, per-level tree indent exact to the pixel, no horizontal overflow), not by vision models — a vision-model self-check falsely approved broken connectors that the user immediately saw. For the hand-drawn gate, `fitSVGTextSizes` (wired into `assembleHTML` after `ensureSVGViewBox`) re-runs the skill's character-coefficient label formula engine-side and shrinks still-overflowing node labels to the largest fitting size (floor 10), so the prompt-side formula is discipline and the engine owns the guarantee.

## Alternatives considered

- **Relax the skill toward hand-drawn SVG.** Rejected: it re-runs the July cycle (expressiveness → overflow/overlap → crackdown). No engine-side font-size floor ever existed — the Go-era "auto-shrink" of the 优化 1–4 plan never shipped as code either; only `ensureSVGViewBox`/`sanitizeSVGVars` and the prompt-side formula landed — and the 2026-07-16 convergence was decided the same day that formula landed, so the prompt discipline alone did not save hand-drawing. A low-effort render model following a prompt-side font-size formula is exactly the original failure mode; the current design pairs the narrow hand-drawn gate with the engine-side `fitSVGTextSizes` floor instead of widening it.
