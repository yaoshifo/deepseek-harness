...
- **提示词**：agent 约定段「白话直讲」计划句把两层映射到两个参数（白话层写进 `plan`、实施细节层写进 `details`）；实施细节层的内涵包含执行分组与并行安排（哪几组并行、哪几组按序），plan-mode 段要求的执行顺序标注因此有了明确落点。自检句不变。
...
**把 `details` 设为必填。** 暂缓：必填可机械保证分层出现，但上游共享工具对所有部署生效。2026-09-10 飞书抽样（4 次退出 3 次内嵌）落地的是更窄的纠正性门禁；必填留作门禁后仍持续失败时的下一级升级（README 已记升级路径）。

**把执行分组写进白话层。** 放弃（用户裁定，2026-09-11）：派发调度属于执行层信息——批准者定「做什么」与验收标准；抽样中也没有任何计划真的把分组放错层。写进白话层还会把实施细节重新塞回分层本要减负的批准面。

## Consequences

- 鲁棒性分层：结构=合同（工具 schema），语义=引导（提示词），失败模式=可检出错层的纠正性拒绝（内嵌细节小节而无附录），其余情形优雅退化（无 `details` 且无内嵌小节 → 卡片退回整段，内容零丢失）。
...
- 小节形态的语义放反（细节以可识别标题写进 `plan`）在评审前就被拒绝门禁拦下；无标题的散文式细节仍渲染整段平铺卡，纳入既有抽样复测（`details` 填写率 + 白话层出现率 + 细节层内的分组标注出现率）。

## Testing

`packages/plan/plan-mode/tests/plan-mode.spec.ts`：带/不带/空白 `details` 三路的 detail 拼接与 intent 形状（含 :1069 精确断言守恒）+ presentCall 两段；拒绝门禁覆盖内嵌小节拒绝（错误文本、评审零调用）、提交 `details` 后放行、普通 `## Details` 小节不误伤。`packages/acp/feishu-bridge/tests/agent-dsh/adapter.spec.ts`：intent.layers 透传与缺席不变。`tests/engine/engine-m3-plan.spec.ts`：分层卡结构（`expanded:false`、面板标题、导出按钮）、无 `layers` 不回归、按层截断。`tests/agent-dsh/adapter-persona.spec.ts`：白话直讲两参数映射措辞逐字钉定。
