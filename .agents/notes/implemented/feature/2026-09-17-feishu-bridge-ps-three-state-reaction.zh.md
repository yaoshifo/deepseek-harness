...
- **Get 与 DONE 累积不撤回。** 否决：残留的 Get 永远读作「还在排队」；monitor 流程的撤回换标是既有模式。
...
- 已接受的竞态（已文档化）：stall-retry 换通道时被 drain 掉的认领事件可能让一个已认领的 steer 留着 Get（欠报不撒谎）；daemon 重启丢失内存记录（陈旧 Get）。stop/cleanup 结算覆盖所有引擎主动的拆除路径。
