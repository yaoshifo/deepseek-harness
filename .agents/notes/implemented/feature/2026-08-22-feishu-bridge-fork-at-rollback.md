...
English | [中文](2026-08-22-feishu-bridge-fork-at-rollback.zh.md)
...
Rollback works for merely-persisted parents and survives a daemon restart between the command and the child's first message — strictly better than the plain `/fork` seed path, which still requires a live parent (that ceiling is unchanged and documented in the package README).
