...
`config.projects[].mcpServers` is a per-project MCP server-name allowlist (absent or empty = unrestricted, no behavior change — Cordis validation materializes absent `Schema.array()` fields as `[]`, so the two forms reach the code identically). When non-empty, the project's adapter denies the `mcp__*` tools of every non-allowed server by visibility mask — MCP connections stay process-global; only what sessions see changes. Three creation funnels carry the mask, all inside `DshAgentAdapter`:
...
- Cordis validation materializes an absent `mcpServers` as `[]`, so "absent" and "configured empty" are indistinguishable and every consumer treats an empty allowlist as unrestricted — the adapter's setup hooks (`allow.length > 0`) and the `/mcp` masked marker apply the same empty guard, so an unconfigured project renders no masked markers.
