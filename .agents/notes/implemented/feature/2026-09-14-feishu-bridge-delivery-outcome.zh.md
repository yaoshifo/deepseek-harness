...
**完整 DeliveryReceipt 移植。** dsh-im 的回执对象、延迟投递 outbox 与历史捞取协调在本批次被否：单渠道、无回执合并消费者，且廉价版本（强杀交货、先投递兜底）已覆盖钉死的痛点。完整延迟系统仍是[对照调研](../../../../packages/acp/feishu-bridge/docs/DSH-IM-COMPARISON.md) §11 总表里的候选行。
