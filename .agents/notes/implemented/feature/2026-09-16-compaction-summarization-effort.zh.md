...
- `BasicCompactionConfig` 新增 `summarizationEffort?: string`（`modelPolicies` 里也可按目标覆盖），与 summarization provider/model 对一同链解析；默认 `''` 省略字段走 provider 默认，保持现状行为。
- `summarizeWithLlm` 把配置的档位经 `ReasoningEffortId()` 传入一次性请求。llm runtime 已校验档位与模型声明的 `reasoning.efforts`、不支持的档位 fail-loud——这正是默认值是 `''` 而非硬编码 `low` 的原因：非推理模型、或档位阶梯不同的模型，硬编码会让每次压缩都失败。
- 以高思考档位跑推理模型的部署应显式配置（live profile 为 deepseek-flash 钉了 `low`，它声明 low/high/max）。

## 考虑过的替代方案

**硬编码 `low`。** 对不声明 efforts 或阶梯不同的模型每次压缩必炸；runtime 的 fail-loud 校验让无条件取值成为部署破坏者。

**继续调大 `maxTokens`。** 治标；思考没有上界，任何帽都是抽签。维持 16384 作为纪要正文本身的余量（实测 ~1.5 万字符）。

## 后果

- 配置的档位作用于该后端的每一次摘要调用（压力触发、溢出恢复、手动 `/compact` 都汇入 `summarize()`），按摘要路由到的模型生效。
...
- 测试：新键的配置解析/覆盖/校验、经 `ReasoningEffortId` 的 wire 传递、未设置时的省略（`compaction-basic.spec.ts`）。
