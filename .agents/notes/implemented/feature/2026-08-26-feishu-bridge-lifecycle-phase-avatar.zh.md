...
Pinned by `tests/feishu/avatar-state.spec.ts`（解析顺序、去重、懒渲染 + 缓存、per-chat 涂改串行化、legacy/bot 回退、先应用后持久化、基线规则）、`tests/feishu/avatar-icon.spec.ts`（设头像时的初始对 + phase 簿记）、`tests/engine/avatar-phase.spec.ts`（askUser 入口/结算矩阵、回合终局错误/成功、best-effort 语义、`/done` 冻结）、`tests/feishu/spawn-evict.spec.ts`（phase 字段持久化 round-trip）、`tests/engine/spawn-family-commands.spec.ts`（`/undone` 恢复基线）、`tests/engine/engine-groupname.spec.ts`（命名兜底仍会盖上按名哈希的图标头像）。
