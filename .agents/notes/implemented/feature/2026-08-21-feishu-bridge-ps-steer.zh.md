...
`tests/engine/misc-commands.spec.ts` 重写 `/ps` 块：mid-turn 断言 `steerCalls` 收到文字、`sendCalls` 为空（表情断言在 /ps 表情套件——2026-09-17 为三态，2026-09-18 起为叠加）；turn 阻塞在权限时仍 steer，`pendingMessages` 不增长、无排队回复；idle 剥前缀穿透。`tests/agent-dsh/adapter-steer.spec.ts`（2 例）：`steer()` 把 user 文本消息路由进 `handle.agent.steer` 而非 followup 队列；`send()` 留在 followup 队列。九处既有桩补 no-op `steer` 以满足接口。
