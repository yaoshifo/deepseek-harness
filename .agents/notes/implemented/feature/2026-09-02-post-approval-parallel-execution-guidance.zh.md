...
- 三个 surface 陈述同一条边界——互不依赖的工作块默认并行、轻量单焦点保持串行：
  ...
  2. 计划模板的标注在当时的两份有门禁拷贝（bundle base patch + bridge patch）里升级为命令式执行顺序声明："state the execution order — independent groups dispatched together as parallel subtask spawns when execution begins, serially dependent groups executed in order"（2026-09-06 起只剩 bridge patch 携带它，[fork 冲突面缩减](../process/2026-09-06-fork-conflict-surface-reduction.zh.md)）。执行策略成为用户审批的计划内容——住在实施细节层，卡片默认折叠、可展开；`bundle-patch.spec` 钉住该句并维持 base≡bridge lockstep。
...
- preset 的执行顺序句不动：三份 preset 拷贝不在 fork 的 live 装配链（web-app 面）且已落后一版；base↔bridge lockstep 门禁覆盖 live 那一对。探索句一族已于 2026-09-03 重新同步到 preset（[单焦点广度判据](2026-09-03-single-focus-breadth-criterion.zh.md)），直至 2026-09-06 缩减把 preset（与 base）回退为上游原文——live 引导只剩 bridge patch 一处（[fork 冲突面缩减](../process/2026-09-06-fork-conflict-surface-reduction.zh.md)）。
...
- 验收：2026-09-02 用户 `/reload` 后复放，四个形状全部通过——含 ≥2 独立组的 plan 审批流把组 A/B 在同一 assistant 消息一起派发（审批边界后先有两个准备步骤）并加 gather 屏障；单组计划 0 派发自实施，且计划正文明确写出执行顺序；显式「并行」回归在同一消息 fan-out 两个 slice（只读任务本就不开评审——预期机制而非缺漏）；非 plan 直执行在首条消息即派发两个 slice、无任何审批。8-31 采样标准仍然适用：单次通过的复放是临时性证据。复放还暴露两个与引导无关的引擎异常（子任务第二次 report 未唤醒空闲父会话——卡 13 分钟后靠用户消息解卡；`/new` 快速连发叠加并发会话时一条用户消息丢失），已记录待另行排查。
