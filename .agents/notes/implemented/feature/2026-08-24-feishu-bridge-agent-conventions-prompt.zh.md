...
代价：每个普通会话多付约 1550 个中文字符的固定系统提示前缀；收尾卡片被忽略的会话会挂到 turn 超时（下一条用户消息仍会作为答案流入，由自由文本条款兜底）。subtask 子会话不再经用户全局指令继承好奇心文本——刻意的作用域收窄，其发现经父会话聚合上报。换来：一份钉在仓库里的自包含行为约定，所有部署 bridge 的机器行为一致，机器本地零配置。
