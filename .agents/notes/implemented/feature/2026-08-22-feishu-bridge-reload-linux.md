...
`/reload` works on Linux deployments with an unchanged reply contract: failures before the restart (build error, missing unit) reply in chat, failures after it (probe timeout) are log-only — the documented ceiling. A probe timeout, and a daemon that dies inside the stability window, tail the last five journal lines into `feishu-bridge-reload.log` for diagnosis. The excerpt must be written `>&2 2>/dev/null`: the shell applies redirections left to right, so `2>/dev/null >&2` copies the descriptor that already points at `/dev/null` and discards the excerpt — the 2026-09-17 dev reload failed in the profile-config validation, and the discarded line left the rollback runbook reading as the cause. `journalctl --user` needs the user bus (`XDG_RUNTIME_DIR`); the systemd unit sets it and a plain terminal has it.

## Testing

`tests/reload-script.spec.ts` gains a `reload.sh on Linux/systemd` suite running on darwin and linux by shadowing `uname`/`systemctl`/`journalctl`/`ps` on PATH (a no-op `sleep` stub lets the 60-iteration probe timeout exhaust in milliseconds): happy path (exactly `cat` + `restart`), `DSH_SESSION_JSONL` refusal with zero systemctl calls, missing-unit fail-loud, probe timeout, stability-window respawn, the rollback runbook's clean/dirty/no-git branches, `FB_RELOAD_FROM_DAEMON=1` walk bypass, and that bypass still refusing daemon-hosted sessions. Two cases pin the failure excerpt: a failed probe and a stability-window death must both show the journal line that names the real error. Real-device smoke per MIGRATION.md (terminal run on the dev server, then chat-side `/reload --skip-build`).
