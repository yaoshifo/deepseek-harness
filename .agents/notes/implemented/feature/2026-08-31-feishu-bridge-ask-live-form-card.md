...
- **Every not-yet-settled question stays interactive and shows its current answer**: single-select rows keep their buttons with a `当前：` line above them, multi-select checkers keep their submit with the current selection ticked, and card-input answers show as `✍️ text`. Once every question is answered the replacement is the read-only terminal card — frozen marks, custom text, no controls, title stamped `· 已全部作答`. Per-question controls are visually scoped so none reads as a whole-card submit: the checker submit labels as `提交第 N 题` on multi-question cards (`提交本题` on single ones, via `CardCheckOptions.submitLabel`; delete-mode pickers keep the generic `提交选择`), and question blocks are separated by dividers on multi-question cards.
...
- **Stale clicks are consumed**: an askq card action with no parked ask replies `AskqStaleCard` (split from `AskqStaleQuestion` in 2026-09-02: the no-parked-ask hint now points at plain chat; `AskqStaleQuestion` remains the question-list-changed hint inside a live ask) instead of leaking the payload.
...
- Chat-text answers sync back to the card: the engine pushes the collected state to card-capable platforms (`syncAskCard`, duck-typed like `sendCard`), which merges it into the callback-side `askqAnswered` map and PATCHes the ask card recorded at send time — the live-form/terminal rendering stays truthful without a callback, and a later click's card replacement keeps the synced state. Card actions skip the sync (their callback response already replaces the card); a missing record or cache entry, or a PATCH failure, is a logged no-op that never breaks answer routing.
