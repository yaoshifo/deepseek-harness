...
- Absolute turn cap: `display.absoluteTurnTimeoutSecs` (unset = 2× idle, 0 disables) → `absoluteTurnMax` → hard cap 3× enforced **on event arrival** in `processInteractiveEvents`, killing with the previously dead `watchdog_reset` message. Arrival-time enforcement is the deliberate TS shape: the event loop has no watchdog goroutine to poll, and the trickle-forever case is exactly the one where events keep arriving; the quiet case is already owned by the stall-retry path (TS's designed primary, where Go's watchdog is only a backstop). Research sessions lift the cap via a faithful `isResearchSession` predicate (assistant flag or research-hub role). The cap clock counts **per turn**: a queued-message takeover resets it (a deliberate deviation from Go's per-run clock; see the [watchdog per-turn reset note](../bug-fix/2026-08-21-feishu-bridge-watchdog-per-turn-reset.md)), and the stall-retry path does not reset it.
