...
Parity with Claude Code means no schema enforcement: frontmatter quality, index-line hooks, dedup-before-save, and deleting wrong memories stay prompt-governed. The plugin enforces only the trust boundary (single-segment file names, normalized to the `.md` suffix per [memory-name-suffix-normalization](../bug-fix/2026-08-17-memory-name-suffix-normalization.md)) and adds harness value where Claude Code does — backfilling `node_type: memory`/`originSessionId` provenance into an existing frontmatter `metadata:` block (additive, never synthesized) and warning after an over-budget `MEMORY.md` write. Pointer lines stay model-authored: `memory_index` maintains one line per call on the model's instruction and never invents titles or hooks — generated hooks would silently degrade recall quality.
