...
- **每道未结算的题保持可交互并显示当前答案**：单选行保留按钮、上方加 `当前：` 行，多选 checker 保留提交、当前选择打勾，卡内文字答案显示为 `✍️ 文本`。全部答完后换卡为只读终态——冻结标记、custom 文本、无控件、标题追加 `· 已全部作答`。每题控件在视觉上作用域化，避免被读成整卡提交：多题卡的 checker 提交按钮标注 `提交第 N 题`（单题卡为 `提交本题`，经 `CardCheckOptions.submitLabel`；delete-mode 选择器保持通用 `提交选择`），多题卡的题目块之间以分隔线分组。
