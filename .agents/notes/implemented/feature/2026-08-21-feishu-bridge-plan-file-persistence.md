...
`tests/engine/plan-file.spec.ts`: helper naming/revision/dedup/dir-creation cases, plus event-loop integration — presentation writes the file with the full plan, `planDir: ''` skips persistence, a default-constructed Engine persists nothing (fail-safe default), a model-written `.claude/plans` file is never overwritten and wins the card, and an unwritable directory falls back to the inline card without throwing; `resolvePlanDir` pins unset→user default, `~` expansion, and `''` passthrough.
