...
- The plan-mode exploration sentence becomes a default with an explicit exception, in all four preset/bundle copies plus the bridge bundle patch's plan-mode override: split any repo-wide scan, cross-cutting audit, or request naming several directions into 2–5 independent angles up front and dispatch them together in one assistant message (read-and-report-only briefs); keep exploration serial only for a single-focus question one or two reads can answer — since 2026-09-03 that exception is sized by the subsystems or directions the answer must cover, not by command count ([single-focus breadth criterion](2026-09-03-single-focus-breadth-criterion.md)). Generic repository copies stay tool-neutral ("background subagent delegations"); the bridge bundle patch override names `feishu_bridge_subtask` — since 2026-09-01 it lives in the bridge package's own `cordis.patch.yml` ([bundle-patch ownership note](../architecture/2026-09-01-feishu-bridge-plan-mode-guidance-in-bundle-patch.md)).
- "group implementation changes by subsystem" now carries "and state the execution order — independent groups dispatched together as parallel subtask spawns when execution begins, serially dependent groups executed in order" — the marking became an imperative execution-order statement when the post-approval consumption gap closed ([post-approval execution guidance](2026-09-02-post-approval-parallel-execution-guidance.md)). The order lives in the plan the user reviews — in the implementation-details layer, collapsed by default on the card and expandable — so the approval gate stays with the user.
- The skill's exclusion boundary narrows to its cost rationale: lightweight single-focus questions (one or two reads/greps) stay serial; multi-direction research defaults to parallel no-group spawns. The 2026-08-21 boundary priced the attended-group surface; unattended spawns have ridden the native continuable seam without creating groups since 2026-08-24, so that cost premise is gone. The skill's plan-mode section states the same boundary as the plan-mode prompt: read-and-report-only exploration spawns dispatch directly during plan mode, while the ExitPlanMode gate scopes to mutating dispatches — children that write files or create worktrees.
...
- **A parallelism nudge in AGENTS.md global instructions.** The plan-mode section already covers the deployment's default (plan) mode, where all the failing cases ran; non-plan direct-execution chats are the remaining uncovered surface, accepted as small. Reversed 2026-09-02 by the [post-approval execution guidance](2026-09-02-post-approval-parallel-execution-guidance.md), which makes the global-instructions boundary one of three surfaces.
...
- Deployment: the bridge bundle patch's plan-mode override replaces the whole section row ([ownership note](../architecture/2026-09-01-feishu-bridge-plan-mode-guidance-in-bundle-patch.md)); the per-profile shims the 2026-08-21 decision added are deleted from both live profiles, so guidance evolution rides the linked package's pull + `/reload` instead of per-machine section re-copies (the 2026-08-21 "delete once the dsh-base release carries the text" condition is superseded: the release text stays tool-neutral, which this composition cannot use).
- Tool-name routing stays split by composition scope: generic repository copies (presets, dsh-base bundle) are tool-neutral; the bridge bundle patch — a repository copy scoped to bridge compositions — names the tool. The [parallel scheduling](../../archived/feature/2026-08-09-parallel-subagent-delegations.md) and [unattended native seam](2026-08-24-feishu-bridge-native-unattended-subtasks.md) mechanisms are untouched.
