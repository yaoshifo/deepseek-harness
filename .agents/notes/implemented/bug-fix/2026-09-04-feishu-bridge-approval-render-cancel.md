...
`routeAskResponse`'s `cancelRenders` became conditional: when a pending ask exists, `kind !== 'questions'`, and `parsePermissionVerdict(content)` is allow or allow-all (card buttons and free text are equivalent), the cancel is skipped — approval settles the ask and resumes the turn, and the pre-ask segment's in-flight render remains a valid delivery (as of 2026-09-09 the exemption is narrowed by render family: plan-card renders cancel on approval and only the speculative reply render keeps the exemption, see [the approval exemption narrowed by render family](2026-09-09-feishu-bridge-plan-render-approval-cancel.md)). Every other response (deny, any answer to questions, an expired button with nothing pending) keeps the Go cancel semantics; the other four `cancelRenders` call sites (new-turn entry, interactive-state cleanup, session stop, card-nav buttons) are unchanged.
...
- Testing: `engine-ask.spec.ts` gained the `routeAskResponse render-cancel semantics` describe, pinning through the public interface (`askUser` + `registerRenderCancel` + `routeAskResponse`) that the permission approval shapes skip the reply-render cancel (`perm:allow`, `perm:allow_all`, free-text 「允许」) and three preserved shapes still cancel (deny, questions answers, expired buttons with nothing pending); the current plan-review semantics (plan render cancelled, reply render kept) live in [the 09-09 narrowing note](2026-09-09-feishu-bridge-plan-render-approval-cancel.md).
