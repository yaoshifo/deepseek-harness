...
- **Reply rendering is length-tiered** (`planRenderDirectLen`, default 2000 runes, 0 = off): at or below the threshold the engine writes the SimpleHTML fragment itself and assembles the template in place — no fork; above it the LLM distillation fork is kept, because the render skill's contract is a one-screen overview, not a copy. The card for mid-length replies now shows the full text.
...
- Tests: `engine-ask-machine.spec.ts` (machine exemption + human/card regressions), `plan-render-direct.spec.ts` (tiers, injectable threshold, render parent lineage), `adapter-agent-message.spec.ts` (relay observation), `subtask-report-dedup.spec.ts` (dedup + engine wiring), `session-save-debounce.spec.ts`, `adapter-oneshot.spec.ts` (poll origin + memory re-injection + parent links), `chatroom-ledger.spec.ts` (heading strip, synthesis archive), `engine-chatroom-supervise.spec.ts` (tick config, breaker cap, completed-gather re-wake), `engine-chatroom-gather.spec.ts`. Merged-tree suites: 198 files, 3444 passed.
- Deployment: host build + `/reload`; production raises `pollMaxConcurrent` to 14 (the role library size) in the profile patch. Live signals: polls with no title-generation calls, mid-length replies with no render forks, `chatroom: supervisor` lines only on genuine stalls.
- Known limits: the dedup record is in-memory only (a restart between direct message and report re-delivers the full text — harmless); direct-tier cards show the verbatim reply rather than a distilled overview; serial-ask breaker notice counts live per entry with no organic reset (the entry's retirement resets them); pre-start poll rows still leak into an abandoned next run's ledger.
