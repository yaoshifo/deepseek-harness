...
The structural mismatch where chatroom state lives on per-Session records but resolves through the per-key active mapping — a `/new` or idle reset in the hub or role chat swapped the active record and silently orphaned moderator/research/barrier flags (gather never completes, personas reset, a role drops out of its room) — is fixed by chat-scoped inheritance: `createLocked` carries lineage, chatroom identity and orchestration, provisioned-assistant, and armed-barrier state onto every replacing record (the previous spawnUserID special case generalized), while conversation-scoped state (one-shot gates, worktrees, history) correctly resets. `/switch` to an older record still resolves through that record's own flags — the remaining edge, accepted. The follow-up tidy commit also deleted 273 i18n keys from unported Go commands (`/allow`/`/yolo`/voice/heartbeat/ws/...), the half-wired `toolResultMeta` projection, and the dead Event fields (`arrivedAt`, `requestID`, `sessionID` dead reads, `gitBranch`, `UserQuestionOption.preview`); the `error` event arm and `errorText` stay — the result events carry `errorText` today.
