...
- ✅ 完成通知页脚对 max-tokens 回合仍显示 `✅ 完成`；对齐该页脚留作后续（它是状态摘要，不是回合的终态主张）。
- 覆盖：adapter 投影（max-tokens 携带、completed 省略）、引擎 finalize（truncated 且非 completed）、预览 `markTruncated`、双语文案标题/颜色、停止按钮与 spinner 隐藏。
