...
权限卡（或 AskUserQuestion 卡）发出时，活进度卡当场终结（转绿、导出按钮从捕获快照起即可用）；批准后的执行开新占位卡继续。预览活跃时交互前文本留在完成卡上——只有预览降级才以纯消息重发，与 Go 一致。Go 在批准时归档 plan 文件的机制（`pendingPlanArchive`，批准时带时间戳后缀复制）未随本次移植——记为独立的迁移缺口。
