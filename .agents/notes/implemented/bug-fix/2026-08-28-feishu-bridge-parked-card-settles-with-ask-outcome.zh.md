...
- 结算头刻意不用绿色：`injectReplyButtons` 会给每个绿头卡追加导出/查看回复按钮，键是 turn 末尾缓存的 export 内容——挂起卡永远没有，按钮会空转。
