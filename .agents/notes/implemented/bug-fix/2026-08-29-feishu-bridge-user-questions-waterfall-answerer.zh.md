...
- 监听器返回持有该会话的 adapter 的 `handleUserQuestion` 结果即认领请求，否则以 `next()` 委托；共享 question routing 仍按每次插件应用恰好注册一个监听器，并把 ask 派发给持有活跃会话的 adapter。该监听器归插件应用所有，而非注册它的 adapter：注销器从不进入任何 adapter 的 disposers，因此单个 adapter 的 dispose 无法移除这个 daemon 级 answerer。
...
- dispose 一个共享路由的 adapter 不影响共享监听器；只有路由 ctx 自身被 dispose 时 Cordis 才会连同所有 adapter 一起移除它。回归测试 dispose 首个共享 adapter 后断言监听器仍为余下 adapter 应答。
