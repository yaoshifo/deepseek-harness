...
- 「后台启动 + 同回合领取」模式定稿的卡上根本不再有后台提示：结算前对账在终态渲染之前丢掉泄漏计数；reader 的 idle 对账则继续清掉它再也无法佐证的计数，让 reader 在一个 idle tick（约 60 秒）内解除武装，而不是拖满 30 分钟宽限。
...
- `tests/engine/engine-events.spec.ts`：结算前对账——泄漏计数不渲染到定稿卡上，仍活着的 job 保留计数与提示。
