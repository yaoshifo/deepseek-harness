...
- **Reconcile against the registry wherever the count is consumed** (`engine.ts`): while `backgroundTasksPending > 0`, ask the registry before any clock or render decision — in the unsolicited reader's idle branch, and once more in the turn's settle path before the terminal render, so a count the registry cannot justify never reaches the settled card (`engine: settled card background count reconciled away`; [duplicate-card note](2026-09-17-feishu-bridge-duplicate-completion-card.md)). Three states: live jobs (`pendingBackgroundJobs()`, running/stopping) keep waiting (49f177d17c semantics, unchanged); settled-unreported jobs (`settledUnreportedBackgroundJobs(since)`, a probe over the same registry cut: settled + `reported === false` + finished after the caller's wait anchor) still owe a notice — the existing grace clock keeps waiting for those (delivery never flips `reported`, so the anchor, not the flag alone, decides what is still owed; the [finishedAt-anchor note](2026-09-17-feishu-bridge-bg-reconcile-finished-at-anchor.md) owns that refinement); live == 0 and inflight == 0 means every counted job settled AND was collected in-turn — a leak, cleared where it was found, under a distinct log line ("reconciled away" vs "grace exhausted"). Every hint write renders the registry's live count, never the count itself (`refreshBackgroundHint`): `💡 N 个后台任务` names jobs still running and no live job means no line, on running cards as well as settled ones (the settle path also logs `engine: settled card background hint cleared`). A `run_in_background` call earns its hint at its own tool result, because the job registers inside the tool body — after `tool/call` reaches the bridge — so a probe at start would miss the job it has to name. A count an owed notice keeps alive is bookkeeping for the reader's grace clock, and a settled card accepts no later render — a hint the registry cannot back would freeze onto it for good (2026-09-18 oc_5677: 💡 6 个后台任务 stood on a settled card holding no running job).
...
- The "background start + collect in-turn" pattern settles its card with no background hint at all: the settle-time reconcile drops the leaked count before the terminal render, the hint names the registry's live jobs wherever a card renders (running cards included), and the reader's idle reconcile still clears a count it can no longer justify so the reader disarms within one idle tick (~60s) instead of the 30-minute grace. A job that settled mid-turn keeps the count (its `finishedAt` is newer than the settle-time anchor) but can no longer be rendered as a running task on the settled card.
...
- Cost: one in-memory registry `list` per hint write (turn start, notice consumption, a run_in_background call's result, a background turn's settlement, and the settling turn) plus one per reader idle tick while a count is pending.
- Drift alarm: the probes anchor `JobSnapshot.ownerSession`/`status`/`reported`/`finishedAt`; a jobs-package change to any of those shapes fails the adapter-projection filter cases loudly.
- The old blind-grace test was retargeted to the in-flight-notice semantics (its no-job stub default now reads as a leak under reconciliation); the reconciliation case carries its own test, and the anchored probe's cases live in the [finishedAt-anchor note](2026-09-17-feishu-bridge-bg-reconcile-finished-at-anchor.md).

## Tests

- `tests/engine/engine-unsolicited.spec.ts`: a leaked count reconciles away at the first idle tick; the retargeted in-flight-notice grace boundary; the live-job grace test unchanged. The hint follows the registry at every write: a notice dropping a slot renders the live count, a started job is named only once its own result registered it, a woken completion turn drops a hint its finished job cannot back, and the write is skipped where there is no card to reach (no preview, a platform without one, or a session already gone).
- `tests/engine/engine-events.spec.ts`: the settle-time reconcile — a leaked count never renders on the settled card, and a still-live job keeps both the count and the hint. The hint's source is the registry's live cut: an owed notice keeps the count but renders no line, a live job's line carries the live count rather than the pending count, and a live job the count no longer tracks still gets its line.
