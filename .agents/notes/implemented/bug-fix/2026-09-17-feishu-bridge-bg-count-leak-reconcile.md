...
- `tests/engine/engine-unsolicited.spec.ts`: a leaked count reconciles away at the first idle tick; the retargeted in-flight-notice grace boundary; the live-job grace test unchanged. The hint follows the registry at every write: a notice dropping a slot renders the live count, a started job is named only once its own result registered it, and a woken completion turn drops a hint its finished job cannot back.
