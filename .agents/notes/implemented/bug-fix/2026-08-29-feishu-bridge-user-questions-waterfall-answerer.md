...
- The listener claims a request by returning the owning adapter's `handleUserQuestion` result and delegates with `next()` otherwise; shared question routing still registers exactly one listener per plugin application and dispatches to the adapter owning the live session.
...
- The shared-routing listener lives on the first adapter's context: disposing that adapter's project while others run removes the listener while `questionRouting.registered` stays true. This shape predates the fix and is unchanged here.
