...
The `__fork__` branch resolves its seed in two steps: the live registry first (its in-memory log is fresher than the write-behind persisted one), then `persistedForkSeed` — `sessionPersistence.inspect(id)` followed by the same seed trim. The trim lived in `trimCompletedTurnPrefix` (now `seedablePrefix`, which additionally cuts and closes a flying turn — see [the flying-turn fork seed note](../feature/2026-08-30-feishu-bridge-flying-turn-fork-seed.md)); both log views carry sequence numbers equal to array indexes, so a prefix sliced at the balanced cut satisfies the seed's contiguous-from-seq-0 contract unchanged. The lineage metadata (`parentSession` + `seedLength`) is now recorded on both paths, not only the live one; when the source is nowhere the behavior is unchanged (warn + fresh session, no chat message).
