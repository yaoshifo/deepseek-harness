...
侧栏摘要流式期间一个刷新间隔内治愈、静默工具期在下个工具事件治愈；`im.message.list` 轮询消失；每条重发墓碑都携带新状态；周期性重发的 churn 病理类——19 条墓碑事故的载体——被结构性消除，但该事故根因仍不归因。合法墓碑数仍等于被压次数：三个活跃群的实测数据显示占主导的位移源是 bridge 自己的头像阶段重绘（样本中每条墓碑都紧跟「开发虾 更新了群头像」系统消息），且侧栏确实会显示这类系统消息、因此必须夺回尾部——要减这类墓碑只能降低重绘频率，该产品杠杆刻意不在本次范围内。账本看不到的位移类别（改名/头像之外的系统消息）在下一次被记活动前保持未治愈。重启行为构造上安全：治愈状态全部易失，新卡必然落在尾部。
