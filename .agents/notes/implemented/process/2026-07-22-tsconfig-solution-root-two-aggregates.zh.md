...
| `tsconfig.base.json` | 共享 compilerOptions 与源码 `paths` 映射；兼任 vitest 配置里 Vite 原生 `resolve.tsconfigPaths` 与 tsx 等 get-tsconfig 消费方的解析映射（不含 `include`，因此对每个导入方都生效） | 否 |
