...
| `tsconfig.base.json` | Shared compilerOptions and the source `paths` map; doubles as the resolution map for Vite's native `resolve.tsconfigPaths` in the vitest configs and for tsx's get-tsconfig consumers (no `include`, so it applies to every importer) | No |
