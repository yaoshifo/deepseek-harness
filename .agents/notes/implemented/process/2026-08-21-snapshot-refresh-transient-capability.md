...
`supportsAcpImagePrompts` ([`packages/acp/acp/src/content.ts`](../../../../packages/acp/acp/src/content.ts)) reports `false` on every transient miss: the `attachments` or `llm` service not yet in the store when the ACP `initialize` handshake runs, or `resolveModelInfo` throwing into its `catch`. Under full-suite load the probe lost that race once. The refresh write-back ([`packages/test-support/session-snapshot/src/suite.ts`](../../../../packages/test-support/session-snapshot/src/suite.ts)) writes the current output and then compares against what it just wrote, so a transient value passes refresh validation and lands in the fixture; the corruption only surfaces on the next replay run.
