...
The validator and compiler therefore share exact semantics: the implicit parameter root is an open object; required keys come only from `required: true`; defaults remain annotations; explicit nested objects honor their declared openness, except that an undeclared key naming a declared property in the opposite key style is a violation with a did-you-mean hint ([style-variant rejection](../bug-fix/2026-08-28-undeclared-key-style-variants.md)) — on model-input arguments the violation is preempted because the key is first normalized to its declared name ([key-style-variant normalization](../bug-fix/2026-08-28-key-style-variant-normalization.md)), so the violation remains for output validation and direct callers; arrays recurse through `items`; scalar literal constraints are type-correct; and `oneOf` accepts exactly one matching branch. Raw-registered tools own their input validation.
