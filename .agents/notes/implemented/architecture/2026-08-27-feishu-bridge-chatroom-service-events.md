...
- C2 continues from here: move the chatroom modules into the new package behind the `./exports` face, and migrate the sessions.json snapshot to a v3 opaque `featureState` section whose survive-reset declaration set must include `researchAssistantKey`/`researchAssistant`/`researchVenv` — `carryChatScopedState` carries them today (session.ts), which the plan document's decision-3 wording misstated.
