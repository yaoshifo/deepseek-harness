...
- The chatroom listener halves live in the chatroom package's `src/engine/chatroom-policy.ts` (moved out of the bridge with C2), registered once process-wide by the chatroom plugin's `apply()` — a separate module because `chatroom-pick.ts` already imports runtime symbols from `chatroom.ts`, and putting the registration there would close an import cycle.
