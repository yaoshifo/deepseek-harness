...
The native seam gains exactly one start-time option: `SubagentStartRequest.cwd`, an optional absolute path that overrides the parent's working directory in the child session header. It is gated behind a new `SubagentCapabilities.cwdOverride` flag, validated (absolute path) in `SubagentRuntime.start` before provider dispatch, and honored by `childSessionMeta` in the in-process driver; every out-of-process backend rejects it fail-loud through `NO_START_CAPABILITIES`. `tool-subagent` exposes it as a config-gated model-facing `cwd` parameter (`allowCwdOverride`, default false — workspace isolation stays the default stance, and a forced cwd on a disabled instance is rejected at execution time like `run_in_background`).
