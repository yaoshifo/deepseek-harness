...
- 症状：同步后 `dsh web` 启动报 loader `Cannot find module …/<新 client 包>/lib/index.js`，或页面报 `Failed to load plugins … missed the module table`（staticLinked 包）→ 做法：typecheck 与 host 面构建不产 client 包运行时产物——跑 `pnpm run build:lib:client` 补 tsdown 的 `lib/index.js`，`pnpm run build:web` 刷 apps/web dist（staticLinked 包如 dockkit 只进 vite shell、不进 module table，dist 旧则浏览器侧 require 落空）。判别：`packages/client/<pkg>/lib/` 只有 types 没 index.js 即中招。daemon（feishu-bridge）不引用 client 包照常运行，勿误判为 daemon 故障（实例：2026-09-08 同步 5 个新 sidebar/open-in-app 包，dsh web 先炸 node 面再炸浏览器面）。
- 症状：改了 fork 包导出的结构化接口声明（`*Like`）却没动生成物 → 做法：`packages/extensions/tool-cordis/src/api-catalog.ts` 内嵌插件导出的声明文本，改完须 `pnpm run gen-cordis-api` 收敛；fork 政策跳过 doc-sync 与生成物门禁、pre-commit 也不跑它，漂移不会被自动拦住（实例：2026-09-11 把桥 `DshAgentLike` 的 `cwd?: unknown` 改成 `string` 修 no-base-to-string，该接口有两处重复声明——监听器内联类型是第二处，改完第一处才暴露）。
- 症状：`git checkout --theirs -- <path>` 跑完 `git diff --diff-filter=U` 仍列出该文件 → 做法：本机 git 版本下该命令只写工作区、不改索引，必须紧接 `git add <同一批文件名>` 才算解完（2026-09-18 实例：16 个生成物/启动文件一次 checkout 后误以为已解，实际索引仍三路未合并）。
- 症状：生成文档（`docs/*.zh.md` 等）冲突时取 `--theirs` 后发现 fork 段落全丢（`git show dev:<path>` 明显更长）→ 做法：中文侧一律不整取上游侧。恢复：对 zh 做三方 `git merge-file -p <ours=dev> <base=merge-base> <theirs=upstream>`，再按语言中立结构从重生成的英文侧同步——单锚点表（capability-seams）按 ctx 键、事件矩阵整表搬 en 行、config-catalog 按 `<a id=…>` 分节（代码块/`Source:` 行取 en，必要时把 `Source:` 译成 `来源：`、把 en 复制的文档链接改写为 `.zh.md`），最后 `verify-translation-pairing --write <owner.md>` 重记配对并重新 `git add`。
