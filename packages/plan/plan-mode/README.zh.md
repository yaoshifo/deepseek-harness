...
| `rejectionHold` | `false` | 评审被拒绝后，`exit_plan_mode` 在本回合剩余时间内失败；你再次开口——下一条消息，或对提问卡的作答——即解除锁定。关闭则保持经典的「修订并立即重发」节奏 |
...
选择 `Keep planning`（可附自由文本反馈）会让 agent 回去修订计划；关闭评审改为发言，则告知 agent 等待你的下一条消息。开启 `rejectionHold: true` 时，拒绝还会让本回合内后续的 `exit_plan_mode` 调用失败，agent 因此以正文回应反馈、仅在你开口后才重新提交修订版计划；你再次开口——新回合、回合中插话，或对提问卡的作答——都会解除锁定。若没有可用的交互评审，`exit_plan_mode` 无法运行，你仍可用 `/plan off` 离开计划模式。
...
开启 `rejectionHold: true` 时，评审被拒绝会记录当前开回合的起始 seq；同一回合内后续的 `exit_plan_mode` 在进入新评审前就以锁定报错失败。锁定有三条解除通路：认领了用户消息的步骤（回合开场消息或回合中 steer 插话）在 pre-step 监听器里清除它；用户在提问卡上作答时经 `user-questions/answered` 事件清除它；后续回合中的调用在入口处按回合比较清除。插件自身那次评审结算绝不算作答——锁定只在评审返回之后才上锁——所以锁不会自己解开。拒绝报错本身携带指令——以正文回应反馈并结束回合；空反馈变体改为询问要改什么。锁定状态是进程本地的：服务重载会丢失它，下一次拒绝会重新上锁。
