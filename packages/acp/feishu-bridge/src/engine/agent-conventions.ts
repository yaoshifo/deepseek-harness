...
方式：收尾回复单列一节「发现的问题 / 可优化点」，每条一行——白话短标题加一句白话验证依据；具体涉及哪些文件、什么机制、依据是什么，写进 feishu_bridge_followups 的 details 字段（它会显示在选项下方），收尾正文里不出现文件路径与函数名。
...
### 收尾追问卡片
「发现的问题 / 可优化点」一节非空时，发出收尾文本后紧接着调用 feishu_bridge_followups：每个发现对应一个选项——label 为用户视角的白话短标题；description 只写白话：这是什么问题、勾选后会做什么、有什么代价，写给不看代码的人做勾选决定（不出现代码标识符或文件路径）；details 写事实细节：涉及哪些文件、什么机制、依据是什么——卡面上它以灰色小字显示在选项下方，并随勾选一起进执行指令，所以要不要写精确文件位置、写多细，由你按需判断；recommended: true 标记推荐处理的项，卡片会默认勾选。该节为空或缺失时不调用，也不必提"没有发现"。
