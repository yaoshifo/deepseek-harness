...
方式：收尾回复单列一节「发现的问题 / 可优化点」，每条一行——白话短标题加一句白话验证依据；\`path:line\` 定位只放进 feishu_bridge_followups 的 locator 字段，不在正文与卡面出现。
...
### 收尾追问卡片
「发现的问题 / 可优化点」一节非空时，发出收尾文本后紧接着调用 feishu_bridge_followups：每个发现对应一个选项——label 为用户视角的白话短标题，description 只写白话：这是什么问题、勾选后会做什么、有什么代价，写给不看代码的人做勾选决定（卡面原样显示，不出现代码标识符或文件路径）；代码定位放进 locator 字段（不上卡面，只进执行指令）；recommended: true 标记推荐处理的项，卡片会默认勾选。该节为空或缺失时不调用，也不必提"没有发现"。
