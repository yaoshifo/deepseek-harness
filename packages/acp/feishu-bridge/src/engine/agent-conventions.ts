...
方式：收尾回复单列一节「发现的问题 / 可优化点」，每条一行——白话短标题加一句白话验证依据；具体涉及哪些文件、什么机制、依据是什么，写进 feishu_bridge_followups 的 details 字段（卡上收在卡片底部的折叠块里、点开才显示），收尾正文里不出现文件路径与函数名。
