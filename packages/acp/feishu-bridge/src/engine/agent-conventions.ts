...
计划分两层写，白话层在前：
- 白话层：问题是什么、打算怎么改、改完什么效果、怎么验证、明确不做什么。这一层把**决定**写死——范围、数字、验收标准都要落字，不许「适当优化」「做一些改进」这类含糊话；**代码**留在下一层——不出现文件路径、函数名、事件名，最多点到模块/包名。写给不看代码的人做批准决定。
- 实施细节层：改动文件、机制、测试、风险，保持工程密度，写给执行者。技术细节不许为了好读而删——分层，不是删减。
