...
- **按需批判性追问（只带质疑点，不塞框架）**：你不是纯传话筒。当某角色的发言有**明显事实错误或逻辑漏洞**时，可再次 action: ask 点名该角色追问——但只把**质疑点**（"你说 X，但 Y 似乎与之矛盾，怎么解释？"）带给它，**不要替它构造分析框架或回答路径**（如"请从凸性/吸收壁/via negativa 三个角度补充"这类填空式引导是禁止的）。框架由角色自己选。**按需而非每次**——发言扎实就不追问，继续编排下一位。
...
export function buildChatroomResearchModeratorPriming(
  ...
  sb.push('1. 调 action: gather 加 research: true，研究任务大意：「请从你的视角研究这个议题的全貌。用你的预配助手（feishu_bridge_subtask 工具 action: send，child 用注入你环境的 $CC_RESEARCH_ASSISTANT_CHILD）下最新数据、跑分析、算关键指标。关键数据、分析脚本、中间结果要让助手存成文件留在助手工作目录（即共享研究工作区），不只打印在对话里——便于复现与归档。基于数据给出你的判断，附关键数据/指标。不要只凭记忆表态——要有实证。默认不出图（文本模型看不懂图），除非用户明确要求可视化。」\n')
  ...
  sb.push('3. 再调 action: gather 加 research: true，任务大意：「上一轮各方结论已写入账本，请先读 SYNTHESIS.md 了解其他人的发现，然后补充/反驳/深挖（可再用助手验证）。」\n')
