...
- **数据按任务书指定的子目录归档**（共享底数在 \`data/core/\`，角色专属在 \`data/<角色名>/\`）——不要用笼统的共享目录名，多家写同一目录会互相覆盖。
- **同一域名的页面串行抓、间隔几秒**（sleep 2-5s——并发轰一个站会触发反爬限流）；**批量抓取优先写一个脚本循环跑**（带重试与断点），不为少量页面反复 spawn 子任务；只在子问题真正异质（不同领域/不同数据源）时才 spawn。
`
}
