...
- 实施细节层写进 details 参数：改动文件、机制、测试、风险，以及执行分组与并行安排（哪几组并行、哪几组按序），保持工程密度，写给执行者。技术细节不许为了好读而删——分层，不是删减。细节小节（如「## 实施细节」）不得拼进 plan 参数——内嵌会被 exit_plan_mode 直接拒绝，要求移入 details 重交。
