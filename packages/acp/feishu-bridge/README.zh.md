...
- **进度卡尾部守卫的治愈延迟与已知边界**：守卫按周期（`streamPreview.tailCheckMs`，默认 5s）收敛而非事件即时，被压住到重发回尾部最长一个周期；用户手动撤回的活跃进度卡会被周期性复活直至回合结束（若成困扰，升级路径是在 `recalled_v1` 钩子把对应 preview 标记 degraded）；线程隔离部署不参与守卫；同一群两个流式 bot 会互相压尾重发（现网单 bot 拓扑不存在该形态）。
