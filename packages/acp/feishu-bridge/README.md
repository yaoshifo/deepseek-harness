...
- **The Go command inventory is only partially ported**: `/shell` and its `!` shortcut landed (Agent Note `feature/2026-08-22-feishu-bridge-shell-command.md`), but ~34 of Go's 52 builtin commands (`/help`, `/whoami`, `/history`, `/show`, `/diff`, `/ps`, …) are not registered; the `message_help` i18n text still advertises them. A command-inventory diff with per-command migrate/cut rulings is deferred to M8.
