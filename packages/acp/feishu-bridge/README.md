...
- **The Go command inventory is only partially ported**: `/shell` + `!`, `/tag`, `/untag`, `/undone`, `/notify`, `/board`, `/help`, and `/ps` landed (Agent Notes `feature/2026-08-22-feishu-bridge-shell-command.md` and `feature/2026-08-22-feishu-bridge-seven-commands.md`); `/help` generates its list from the registered handlers, so it cannot drift. ~27 of Go's 52 builtin commands remain unported (`/whoami`, `/history`, `/current`, `/search`, `/delete`, `/name`, `/memory`, `/model`, `/reasoning`, `/mode`, `/lang`, `/quiet`, `/tts`, `/allow`, `/skills`, `/config`, `/show`, `/diff`, …; upgrade/restart/web/doctor/version are D-class cuts). A per-command migrate/cut ruling pass is deferred to M8.
