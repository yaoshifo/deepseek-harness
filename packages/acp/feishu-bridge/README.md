...
- **/fork can only seed from a live parent session**: Go copies completed turns from the persisted log, while the TS side degrades to a brand-new session when the parent is not in memory (the commented ceiling on adapter startSession). The quoted-message rollback fork (`__forkat__`, reply to a message + `/fork`) is exempt: it locates and truncates through the sessionPersistence service, so a merely-persisted parent works.
