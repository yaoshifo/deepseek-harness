...
- **Inbound voice messages are silently dropped**: Go transcribed them via `[speech]` (Whisper-compatible providers) before feeding the agent; the TS platform drops `audio` messages (the `voice_*` i18n strings are ported-but-unwired residue). Porting `speech.go` or replying a "voice not enabled" notice is the pending ruling (2026-08-21 audit).
- **Agent failures surface raw and unclassified**: Go's failure_classify.go (seven error classes driving user-facing text and retry decisions) plus redact/ (secret scrubbing before display) are not ported; the raw error text reaches the chat.
...
- **The Go command inventory is intentionally partial**: `/shell` + `!`, `/tag`, `/untag`, `/undone`, `/notify`, `/board`, `/help`, and `/ps` landed (Agent Notes `feature/2026-08-20-feishu-bridge-shell-command.md` and `feature/2026-08-20-feishu-bridge-seven-commands.md`); `/help` generates its list from the registered handlers, so it cannot drift. The remaining ~27 of Go's 53 builtin commands are a deliberate curation (user ruling 2026-08-21): upgrade/restart/web/doctor/version are D-class cuts, and the rest (`/whoami`, `/history`, `/current`, `/search`, `/delete`, `/name`, `/memory`, `/model`, `/reasoning`, `/mode`, `/lang`, `/quiet`, `/tts`, `/allow`, `/config`, `/show`, `/diff`, …) stay unported by design; `/tts` additionally depends on the pending voice-capability ruling.
