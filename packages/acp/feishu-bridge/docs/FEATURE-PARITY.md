...
| 9 | 全局 Providers：跨项目共享模型配置 + /provider 切换 | ✅ | M7-c | /provider list/switch/current/clear + provider_shortcuts（/strong 等）+ 切换持久化（project state）+ provider 卡（2026-08-28：卡片平台裸 /provider 渲染可点选列表，行按钮 `act:/provider <name>` 即点即切并原地刷新，帮助卡行 `nav:/provider` 原地打开）；add/remove/presets 不迁移——provider = profile 命名路由，运行时不可创建 |
