...
| 63 | 后台任务可视化：前台进度卡「💡 N 个后台任务」提示 + 未请求回合 header「🔄 后台任务完成，正在处理...」 | ✅ | M8 | 引擎接线已落地：`tool/call` 检测 `run_in_background` → `engine.ts` `backgroundTasksPending` 计数 → `setBackgroundHint` 设/清（完成递减随 unsolicited reader；engine-unsolicited.spec 覆盖计数归零）。渲染：运行态卡提示经结构化 `bgTaskHint` 字段渲染在「⏹ 停止执行」按钮行内（`injectStopButton`，2026-08-24）；终态卡（green/red）保留正文行承载提示；用户停止卡（⏹ 已停止）不重注入提示 |
