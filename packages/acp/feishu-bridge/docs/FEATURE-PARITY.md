...
| 9 | 全局 Providers：跨项目共享模型配置 + /provider 切换 | ✅ | M7-c | /provider list/switch/current/clear + provider_shortcuts（/strong 等）+ 切换持久化（project state）+ provider 卡（2026-08-28：卡片平台裸 /provider 渲染可点选列表，行按钮 `act:/provider <name>` 即点即切并原地刷新，帮助卡行 `nav:/provider -r` 原地打开；同日补模式行：可选热切换 `-r` 保留上下文；2026-09-03 起卡默认热切换——热切换按钮居左、路由行值带 `-r`，见 provider-card Agent Note）；add/remove/presets 不迁移——provider = profile 命名路由，运行时不可创建 |
...
| 35 | Pin 每条用户消息到 Pin 面板（spawn 群） | ✅ | M4 | MessagePinAppender；子项 #35a（spawn 群反馈精简）：topnotice 门控已对齐（spawn 群默认关）；表情链裁定不迁（2026-08-21 用户裁定，Go engine_events.go:1370/4981/5317、engine_predict.go:81），常规反馈由进度卡/完成卡承担；平台表情通路仅 /ps 三态认领反应（Get→DONE→CrossMark，cancelEmoji 默认 CrossMark、'none' 禁用；[Agent Note](../../../../.agents/notes/implemented/feature/2026-09-17-feishu-bridge-ps-three-state-reaction.md)） |
...
| 63 | 后台任务可视化：前台进度卡「💡 N 个后台任务」提示 + 未请求回合 header「🔄 后台任务完成，正在处理...」 | ✅ | M8 | 引擎接线已落地：`tool/call` 检测 `run_in_background` → `engine.ts` `backgroundTasksPending` 计数 → `setBackgroundHint` 设/清。递减两路对齐 dsh 投递机制（2026-09-15 oc_1b7e 修复）：空闲 owner 的通知由 `followup` 唤醒新回合、回合结算时递减（engine-unsolicited.spec 覆盖）；忙碌 owner 的通知被塞进运行中回合（next-step splice，无唤醒回合），adapter 将 `agent/inbox/spliced`（tool-jobs notice）投影为 `bg_task_notice` 事件、送达即递减并按通知 id 去重（adapter-projection.spec + engine-unsolicited.spec 覆盖）。宽限耗尽的善后对齐注册表事实（2026-09-16 oc_3c16b 修复，Agent Note `2026-09-16-feishu-bridge-bg-grace-live-job`）：`AgentSession.pendingBackgroundJobs()`（adapter 查 `JobRegistry.list` 按 owner+running/stopping 过滤）先于放弃被问——有活 job 则 reader 续等保通知，无活 job 才清计数并同步清终态卡提示；idle reaper 的保护同样计入活 job。等待期间每个 idle tick 先做注册表对账（2026-09-17 oc_f85284 修复），结算渲染前在 `handleResultEvent` 再对账一次（2026-09-17 oc_f7b306 修复，62494025c9）——注册表无法佐证的计数不进终态卡：已结算未回报且结算晚于等待锚点（`settledUnreportedBackgroundJobs(since)`，settled+unreported+finishedAt>锚点；reader 传 `bgWaitStartedAt`、结算传 `timing.turnStart`——通知投递不翻转 `reported`，锚点之前结算的 job 不再永久计为在途，2026-09-17 finishedAt 锚定修复）= 通知在途，按上述宽限语义等；全结算且已领取（settled+reported——`job_output` wait/read、kill 已把终态交给模型，tool-jobs 抑制其通知）= 泄漏计数，首个 idle tick 立即清，不再拖满 30 分钟宽限。渲染：运行态卡提示经结构化 `bgTaskHint` 字段渲染在「⏹ 停止执行」按钮行内（`injectStopButton`，2026-08-24）；终态卡（green/red）保留正文行承载提示；用户停止卡（⏹ 已停止）不重注入提示 |
