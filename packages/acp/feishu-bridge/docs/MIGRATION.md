...
**M8 前补充 6：权限解决后的预览卡重启（2026-08-20，测试先行，+1 测试，1844 全绿）**：用户真机反馈——plan 卡（ExitPlanMode 审批）批准后，后续执行的 tool progress 不开新卡、继续 PATCH plan 之前的旧进度卡。根因：Go 交互循环 `pending.Resolved` 之后的块（engine_events.go ~4410-4482）把每次权限解决视为阶段边界——flush 未投递文本段 → `completeAndDetach` 旧卡 → 新建 sp/cp → 重绑 active preview → 预建执行阶段占位卡 → 重置 textParts/segmentStart/toolCount/silentHold；TS 的 `permission_request` 分支只移植了重置半边，sp/cp 旧句柄被继续复用。落地：在 `processInteractiveEvents` 权限解决点保形补齐整块（sp/cp 改 `let`；`state.preview` 同步；覆盖 plan 审批/普通审批/AskUserQuestion 回答，与 Go 一致）。**顺带发现的独立缺口未随迁**：Go 批准时归档 plan 文件（`pendingPlanArchive`，带时间戳后缀复制）TS 无对应。Agent Note `bug-fix/2026-08-20-feishu-bridge-post-permission-card-restart.md`（中英+sidecar）。测试：`PostPermissionCardRestart`（tool_use → write 权限 → resolve → tool_use → result，断言两次 preview start + 批准后 PATCH 只落新句柄）。**留日常验证**：真机 plan 模式 turn，plan 卡前有工具进度，批准后执行应开新进度卡。
