...
| `[stream_preview]` `interval_ms` / `min_delta_chars` / `max_chars` | 插件行 `config.streamPreview.intervalMs` / `.minDeltaChars` / `.maxChars` | 三者只作用于「出现思考/工具之前」的纯文本窗口（`appendText` 路径）；进入进度模式后节流改用 `progressFlushIntervalMs`，实时播报段的长度上限是固定常量（2026-09-14 审计 F4 修正说明） |
