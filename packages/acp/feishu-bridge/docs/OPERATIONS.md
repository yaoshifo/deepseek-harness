...
- `cordis.patch.yml` 永远不会被 install 覆盖：它是运行期热改的配置层。生效方式由桥 bundle 的 `hmr` 行决定（该行 `disabled: true`）：改动经 `/reload` 重启生效，`reload.sh` 重启前先校验配置（§3.3）；删除该行则回到 Cordis HMR 即时热载。
...
| `[projects.agent.options]` `mode` | `config.projects[].agent.mode` | `'plan'` = 每会话默认计划模式；按群覆盖：`/spawn --plan` / `--default`（也适用于 `/fork`）。Go 的 `/mode` 单次覆盖命令未迁移（2026-08-21 裁定）——运行中切换由 `/plan`（`/plan` 进、`/plan off` 出、`/plan <任务>` 进并立刻按该任务出计划）承担，作用于当前活会话，与待下一次启动才生效的一次性覆盖语义不同（2026-09-20） |
...
| `[projects.platforms.options]` `allow_from` / `group_only` / `group_reply_all` / `share_session_in_channel` / `reply_to_trigger` / `respond_to_at_everyone_and_here` / `enable_feishu_card` / `active_tag_name` | `config.projects[].feishu.allowFrom` / `.groupOnly` / — / `.shareSessionInChannel` / `.replyToTrigger` / `.respondToAtEveryoneAndHere` / `.enableFeishuCard` / `.activeTagName` | 2026-08-21 补齐接线（此前机制在但配置不可达）；`group_reply_all` 由 `features.allowChat` 承担；`resolve_mentions` 随 mention resolution 未移植。`progress_style` / `.progressStyle` 已移除（结构化进度卡链路在飞书平台永不可达，2026-09-14 审计 F3）——配置里残留该键会在加载时报错，请从配置中删除 |
| `dir_scan_paths` | `config.projects[].dirScanPaths` | /dir 子目录扫描建议列表 + 裸名解析（~ 展开；M7-d #3） |
| —（无 Go 对应，新增） | `config.projects[].mcpServers` | MCP server 名允许列表：配置后该项目会话只看到这些 server 的 `mcp__*` 工具（含 subtask 子会话与 one-shot 查询），其余 MCP server 的工具从模型请求中掩掉；缺省不限制。MCP 连接仍由 profile 全局挂载，此处只管可见性 |
...
| `[stream_preview]` `interval_ms` / `min_delta_chars` / `max_chars` | 插件行 `config.streamPreview.intervalMs` / `.minDeltaChars` / `.maxChars` | 三者只作用于「出现思考/工具之前」的纯文本窗口（`appendText` 路径）；进入进度模式后节流改用 `progressFlushIntervalMs`，实时播报段的长度上限用 `maxAnalysisChars`（2026-09-14 审计 F4 修正说明） |
| —（TS 新增） | 插件行 `config.streamPreview.progressFlushIntervalMs` | 进度卡最小 PATCH 间隔（ms），缺省 300 = 原写死值；`0` 关闭节流（每次内容变化都 PATCH）。长任务里用户真正盯着的那张卡的刷新节奏，此前不可配（2026-09-14 审计 F4） |
| —（TS 新增） | 插件行 `config.streamPreview.maxAnalysisChars` | 进度卡实时播报段的长度上限（字符），缺省 6000 = 原写死值；超限后卡面显示截断注记、完整回复走带外投递（Feishu 11310 防线）（2026-09-14 审计 F4） |
| —（TS 新增） | 插件行 `config.subtask.reportMaxChars` | 子任务汇报注入父 agent 上下文的码点上限（默认 8192）：超限汇报三处同口径截为头 2/3 + 尾 1/3 + 说明行（唤醒、gather banking、人侧卡片），全文落盘 `<会话存储目录>/subtask-reports/` 供父 agent 按需 read；落盘失败降级为指向子会话的「未落盘」文案（可用 send 追问找回），绝不抛异常；去重哈希取原文。`0`/缺省保持默认，低于 1024 加载期报错（2026-09-16） |
...
| `[chatroom]` / `[subtask]` / `[spawn]` / `[group_name]` / `[plan_render]` / cron / relay / `[projects.monitor]` / `usage_providers` | 插件行 `config.chatroom` / `config.subtask` / `config.spawn` / `config.projects[].groupName` / `.planRender` / `config.cron` / `config.relay` / `config.projects[].monitor` / `config.usageProviders` | 各域 M5–M7 已落地；键名 camelCase |
...
- TS 代码改动（一键，双平台）：`packages/acp/feishu-bridge/reload.sh`——§1.3 两步构建 → 重启 daemon（macOS：`launchctl unload`/`load` + stdout/stderr 日志轮换；Linux：单条 `systemctl --user restart`，原子操作无空窗）→ WS 就绪探活（macOS 查轮换后的 stdout 日志，Linux 查 journal）；`--skip-build` 跳过构建（构建已在别处完成时）；`--force-build` 强制重建。默认还会**复用上次构建**：跟踪面（HEAD + 已跟踪改动；未跟踪草稿不计）与上次成功重启时一致、且入口产物仍在时，跳过构建直接进预检与重启——profile yml 改动不需要新产物，这类重载从分钟级（dev 服务器 4 核上约 5 分 16 秒）降到十几秒；拉了代码、改了文件或切了 commit 都会改变该状态，照旧重建。戳记写在 `$LOG_DIR/feishu-bridge-build-stamp`，只在重启 + WS 探活成功后写入，失败路径永不留下可复用的戳。在 daemon 内的会话里执行会被拒绝（重启会中断自身 turn；由 bash 环境里的 `DSH_SESSION_JSONL` 判定——agent 沙箱会把 `XPC_SERVICE_NAME` 改写成字面量 `0`、并拒绝 `ps`，两者都不可用），须从普通终端跑；进行中 turn 会回滚到最后完整 turn。macOS 上 unload 与 load 之间脚本若因任何原因退出，会自动重试 `launchctl load` 恢复服务。探活超时（30 秒无 `ws client ready`）时，脚本在报错后打印回滚指引：基于重启前捕获的 git 状态（commit + 是否 dirty）给出 stash（仅 dirty 时）→ `git checkout` → 重建 → `--skip-build` 重启的完整命令序列；脚本从不自动改写工作区（单树 dogfood 下主 clone 是并行 agent 会话的活跃工作区），序列由运维在普通终端粘贴执行。
- 同一效果的聊天入口：admin 用户发 `/reload [--skip-build|--force-build]`（TS 原生命令，无 Go 对应）。daemon 以 detached 子进程运行上述脚本并带 `FB_RELOAD_FROM_DAEMON=1`——该变量只豁免脚本的 ppid 守卫（detached 子进程正是该守卫想近似的安全场景），`DSH_SESSION_JSONL` 守卫不豁免。macOS 直接 setsid spawn（launchd teardown 打不到 setsid 子进程）；Linux 经 `systemd-run --user --scope` 生成兄弟 scope 单元——setsid 出不了 `feishu-bridge.service` 的 cgroup，`systemctl --user restart`（KillMode=control-group）会连带杀掉脚本造成重启成功但误报失败（2026-08-22 dev 事故）。输出追加到 `$LOG_DIR`（默认 `~/.dsh`）的 `feishu-bridge-reload.log`（超过 5MB 时轮转为 `.1.gz` 并清空，只留一代——dev 服务器曾攒到 25MB/26 万行）；进行中仅允许一个 reload；脚本在重启之前失败（构建失败、plist/systemd unit 缺失）时 daemon 未重启、用户会收到失败回复。
- 完成通知：spawn 前 handler 写 `$LOG_DIR/feishu-bridge-reload-pending.json`（pid、engine、platform、replyCtx、at）；新 daemon 所有平台就绪后读取标记，按（项目名 → 平台名）两级匹配定位当初发「已启动」回复的同一平台——未配 tag 的部署所有平台默认名都是 `feishu`，只按平台名匹配会选中不在群里的 bot 而被飞书 230002 拒绝（2026-08-22 dev 事故）——把「✅ Reload 完成」回复到原 `/reload` 消息上并删除标记，通知由新进程发出、能发出即证明重启落地。完成通知送达后，若全局工具视图里的 mcp-client 工具总数超过固定线（20 个，`MCP_SURFACE_REMINDER_ABOVE_TOOLS`），同一平台会再跟一条「MCP 工具有点多」提醒（总数 + 每 server 最重在前的分布 + 禁用建议；只提醒不拦截，加载行为不变；提醒失败只 warn 不影响通知与标记）。同 pid（HMR 插件重载在 reload 飞行中重跑 `apply()`）跳过且保留标记；超过 15 分钟视为陈旧静默丢弃；项目/平台已不在配置、标记损坏、发送失败均 warn 后丢弃。残留边界：reload 构建期间 daemon 独立崩溃由 systemd 拉起，也会收到通知（daemon 确实重启了，文案不声称构建结果，详情以日志为准）。daemon 已重启后才暴露的失败（如 WS 探活超时）仍只有日志留痕，无失败回复。
- 配置校验前置：构建完成后、任何 stop/restart 之前，脚本以 `dsh --profile feishu-bridge --dump-config` 解析 profile 各 patch 层（不 boot、不连飞书）。坏 `cordis.patch.yml`（YAML 语法、非数组、条目非映射、文件不可读）在旧 daemon 仍在运行时被拦截——daemon 保持当前配置继续跑，群里收到 `/reload` 失败回复，错误尾部在脚本输出与 `$LOG_DIR/feishu-bridge-config-check.err`。局限：dump 模式不求值 `!!js`、不校验插件 Config schema、不解析插件名——这类错误仍只在重启后的 boot 才暴露（fail-loud 崩溃，systemd 按 RestartSec 重试；修好文件即自愈）。该步骤会重写 profile 的 `cordis.yml` 空根文件，仅因本部署无任何 config watcher（bundle 已禁 `hmr` 行）而惰性。
- profile yml / 插件 config 改动：与 TS 改动同一入口 `/reload`（重启重读磁盘）。桥 bundle 禁用的 `hmr` 行关闭 Cordis HMR 即时热载（2026-09-18 起是唯一开关；此前的 `DSH_CONFIG_HMR_DISABLED` 环境变量随上游移除 launcher 兜底监视器而退役）；删除该行则回到事务热载、无需重启。
