...
const en: Record<string, string> = {
  ...
  chatroom_research_progress_waiting: 'Waiting on: %s (%d min elapsed)',
  chatroom_serial_ask_restarted: 'Process restart detected: your question to %s before the restart (%s) never got its answer — the answering turn died with the process. Re-ask the role or move on.',
  chatroom_role_supervisor_wake: '[Supervisor] Role %s has not answered your question from %d minutes ago (last state: %s). You may follow up, skip the role and continue, or wrap up; without action this reminder repeats next cycle.',
  ...
  chatroom_pick_selected_count: '%d selected / limit %d',
  ...
  chatroom_topic_pick_custom_ph: 'Or type your own topic',
  chatroom_topic_pick_custom_btn: 'Start with this topic',
  ...
  chatroom_poll_in_flight: 'A lightning-round poll is still in flight — start the next one after this round completes or times out and wakes you.',
  chatroom_poll_card_opening: 'Lightning round · every role statement',
  chatroom_poll_card_closing: 'Closing sweep · sidelined-role statements',
  chatroom_poll_gather_blocked: 'A parallel gather is in progress — do not poll yet: the statements would wake you mid-round and tangle the two barriers. Poll after the round wakes you.',
...
const zh: Record<string, string> = {
  ...
  chatroom_research_progress_waiting: '等待中：%s（已进行 %d 分钟）',
  chatroom_serial_ask_restarted: '检测到进程重启：你重启前对 %s 的提问（%s）未获回答——作答回合已随进程终止。可重问该角色或直接推进。',
  chatroom_role_supervisor_wake: '[监督] 角色 %s 对你 %d 分钟前的提问仍无响应（最近状态：%s）。可追问该角色、跳过它继续、或收尾；无动作我会在下个周期再次提醒。',
  ...
  chatroom_pick_selected_count: '已选 %d / 上限 %d 个',
  ...
  chatroom_topic_pick_custom_ph: '或直接输入你的题目',
  chatroom_topic_pick_custom_btn: '用这个题目开始',
  ...
  chatroom_poll_in_flight: '上一轮快答仍在进行中——等本轮收齐或超时唤醒你后再发起。',
  chatroom_poll_card_opening: '闪电轮 · 全员快答',
  chatroom_poll_card_closing: '收尾补盲 · 场外角色表态',
  chatroom_poll_gather_blocked: '并行收集进行中——先不要 poll：表态会打断本轮屏障。等收齐/超时唤醒你后再发起快答。',
...
export const Msg = {
  ...
  ChatroomSerialAskRestarted: 'chatroom_serial_ask_restarted',
  ChatroomResearchProgressWaiting: 'chatroom_research_progress_waiting',
  ChatroomRoleSupervisorWake: 'chatroom_role_supervisor_wake',
  ...
  ChatroomTopicPickCustomPH: 'chatroom_topic_pick_custom_ph',
  ChatroomTopicPickCustomBtn: 'chatroom_topic_pick_custom_btn',
  ...
  ChatroomPollInFlight: 'chatroom_poll_in_flight',
  ChatroomPollCardOpening: 'chatroom_poll_card_opening',
  ChatroomPollCardClosing: 'chatroom_poll_card_closing',
  ChatroomPollGatherBlocked: 'chatroom_poll_gather_blocked',
