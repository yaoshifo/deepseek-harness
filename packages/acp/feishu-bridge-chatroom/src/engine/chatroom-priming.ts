...
import type { ChatroomLedgerPrior } from './chatroom-ledger.ts'
...
/** Extra context the plain moderator priming can carry (both optional). */
export interface ChatroomModeratorPrimingContext {
  /** A prior chatroom this one continues; omitted starts a fresh discussion. */
  prior?: ChatroomLedgerPrior
  /** A shared research workspace with recorded fetches; '' or omitted omits the section. */
  researchWs?: string
}

/** The prior-screening section shared by both moderator primings. */
function priorScreeningSection(prior: ChatroomLedgerPrior, beforeWhat: string): string {
  return `\n## 前情（继承自 ${prior.topic}，未经本次讨论验证）\n上一个聊天室的判断在 ${prior.dir}（SYNTHESIS.md 综述 / SUBPROBLEMS.md 进度 / RECORD.md 记录 / REPORT.md 结论；账本前情区已写入同一指针）。${beforeWhat}先做甄别：Read 前情目录的 SYNTHESIS.md 与 REPORT.md，把每条判断分类——直接采信 / 复核后采信 / 存疑 / 推翻；**采信的部分**用 ${TOOL} 工具（action: note）写进账本综述段并标注「继承自 ${prior.topic}」；存疑项作为开放问题带入讨论；发现错误用「修正：<错误>→<更正>」显式记入综述，不静默沿用也不静默抛弃。复核用**新的独立证据**——复读前情依据的同一批数据只是循环印证，不是验证。\n`
}
 ...
 * @param ctx - Optional prior chatroom and shared research workspace.
 * @returns the moderator wake prompt.
 */
export function buildChatroomModeratorPriming(
  topic: string, roles: ChatroomRole[], ledgerDir: string, ctx?: ChatroomModeratorPrimingContext,
): string {
  const sb: string[] = []
  ...
  if (ctx?.prior !== undefined) {
    sb.push(priorScreeningSection(ctx.prior, '进入阶段 1 之前'))
  }
  if (ctx?.researchWs !== undefined && ctx.researchWs !== '') {
    sb.push(`\n## 共享研究数据（历史聊天室下取，复用前先甄别）\n共享研究工作区 ${ctx.researchWs} 存有历史研究聊天室下取的数据：抓取台账 ${ctx.researchWs}/DATA_LEDGER.md（data/core/ 为公共数据、data/<角色名>/ 为角色专属）。议题相关时先查台账的 来源/口径/抓取时间 三列判断适配性——口径不符的数据再新也是错的；对结论起支撑的关键数据让角色 spot-check 源头或对独立源；确认适配后再在任务里给角色指出具体文件路径。\n`)
  }
  sb.push(`
## 工具（调 ${TOOL} 工具）
...
11. 综合发言后按「## 何时收尾」判断：图景已完整时，先用 ask_user_question 问用户是否结束；用户确认后再调 action: end 收尾，给出结构化总结。**若用户此前选过「出一份深度学术版」**，保持学术结构化总结：**综合出的完整图景、各视角的贡献、仍未解决的开放问题（明确交回人类定夺）**；**否则**走白话直讲：直接讲事情本身，把术语展开成大白话（如「交付契约错配」→「交付的定义跟任务对不上」），绕不开的名词就地用一句大白话说明，用具体的事实和数字做支撑；拆 2-3 核心点；分歧仍显式标出但不用学术表述。不要假装分歧已被消解。
...
1. 核心图景直讲（顶部第一屏，默认展开）：直接讲事情本身——这个讨论在解决什么问题、主要判断是什么、各方大致倾向哪里、在什么地方仍有诚实分歧。2-4 句大白话；把术语展开成大白话（如「交付契约错配」→「交付的定义跟任务对不上」），绕不开的名词就地用一句大白话说明。有定论处给定论，有分歧处显式标出分歧，不要为了干脆而假装分歧已消解。
2. 拆 2-3 个核心门槛（默认展开）：把图景拆成 2-3 个「要懂这件事必须跨过的门槛」。每个 = 一句大白话标题 + 一个最小实例（用讨论里真实的具体事实、数字或场景落地）+ 折叠的专业支撑细节（保真，不降级准确性）。其中【违反直觉】的门槛标 ⚠ 反直觉，并多给一句「为什么这件事反直觉 / 常人会怎么误判」。
...
原则：信息不丢，只是全部换成大白话——直接讲事情本身、用具体事实和数字跨过门槛、用分歧清单标出边界。用户只看第一屏就懂大意，零操作。完成后调 feishu_bridge_subtask 工具 action: report，message: \"HTML 已生成：<path>\"』。**用子任务是为了隔离 html 渲染，不污染你的 context**；子任务无 worktree、会话日志持久保留，chatroom end 时自动回收。
...
## 收尾总结
调 ${TOOL} 工具 action: end，给出结构化总结；总结文字同时用 ${TOOL} 工具（action: note 加 section: report）写进账本 REPORT.md 落盘——它是本次讨论的结论文本，供后续聊天室与 history 查询引用。**若用户此前选过「出一份深度学术版」**，保持学术结构化总结：**综合出的完整图景、各视角的贡献、仍未解决的开放问题（明确交回人类定夺）**；**否则**走白话直讲：直接讲事情本身，把术语展开成大白话（如「交付契约错配」→「交付的定义跟任务对不上」），绕不开的名词就地用一句大白话说明，用具体的事实和数字做支撑；拆 2-3 核心点；分歧仍显式标出但不用学术表述。不要假装分歧已被消解。
...
export function buildChatroomResearchModeratorPriming(
  ...
  if (prior !== undefined) {
    sb.push(priorScreeningSection(prior, '进入第 1 轮之前'))
  }
  sb.push(`\n## 工具（调 ${TOOL} 工具）\n`)
  ...
  sb.push('- action: note 加 section: report（message: \"<收尾总结>\"）—— 收尾时把总结写进 REPORT.md。\n')
  sb.push('- action: end —— 收尾。\n')
  sb.push('- 原生 **ask_user_question** 工具可向用户发飞书卡片提问。\n')
  sb.push('\n## 研究流程\n\n')
  sb.push('### 预备：澄清研究背景（数据需求清单之前）\n')
  sb.push('0. 先判断是否需要澄清：议题明确、且你系统提示词里的「用户背景」段（若有）已足够支撑有用研究时，直接进步骤 1。否则调 action: gather（**不加 research: true**——普通收集轮），问题大意：「从你的人设视角，要让这次研究对用户有用，需要向用户澄清哪些背景/约束/目标（研究目的、受众、已知偏好、硬约束）？若需要，把问题整理成 2-4 个多选题（含候选项）；若你已掌握的背景足够，回复『无需追问』。」角色应当分钟级回复，别在这轮等助手。engine 收齐后唤醒你。\n')
  sb.push('0.5. 合并去重各角色的澄清建议，调原生 **ask_user_question（multi_select: true）**发给用户；用户回答后用 action: note 把回答整理写入账本综述段（标注「用户背景与约束」）——角色下一轮读账本即可见。仍有角色认为关键背景缺失可再来一轮，**最多 2 轮**；到顶后把剩余疑问作为假设标注进综述段继续，不要卡死。已注入的「用户背景」段为底：只问它没覆盖且与本次议题相关的缺口，不要重复问已知信息。\n')
  sb.push('### 预备：数据需求清单 + 公共数据预取（第 1 轮之前）\n')
  sb.push('1. 调 action: gather（**不加 research: true**——普通收集轮，角色直接回复、不开助手），问题大意：「从你的人设视角，研究这个议题的全貌需要哪些底层数据？只列清单，不派助手、不下数据、不做研究——凭你的视角判断。每项写：数据/序列名、时间窗口、首选权威来源。」这是纯判断轮，角色应当分钟级回复，别在这轮等助手。engine 收齐后唤醒你（20 分钟超时兜底，超时带已到的回复）——用已到的清单加你自己的判断继续，别卡死。\n')
  if (researchWs !== '') {
    ...
    sb.push('   然后派数据管家（feishu_bridge_subtask，action: send，child 用 "assistant"）：「下取以下公共核心数据到共享工作区 data/core/ 子目录：<合并清单>。抓取优先按研究 playbook 已验证配方打（速查表里已验证的官方端点打法直接照抄）；按数据源/序列拆成 3-6 个并行子任务（每源一个，不是每页一个——机械抓取优先脚本循环，带重试与断点）；拆子任务时把『同一域名串行抓、间隔几秒（sleep 2-5s，防反爬限流）』写进每个子任务 brief。每抓完一个数据集，用 bash echo 在 DATA_LEDGER.md 追加一行（| 数据/序列 | 本地文件 | 抓取时间 | 来源/口径 | 抓取者 |；文件不存在先写表头行）。完成后 report 清单。」\n')
  ...
  const round1Shared = researchWs !== ''
    ...
    : ''
  sb.push(`2. 调 action: gather 加 research: true，研究任务大意：「请从你的视角研究这个议题的全貌。${round1Shared}用你的预配助手（feishu_bridge_subtask 工具 action: send，child 用 "assistant"）下你还缺的数据、跑分析、算关键指标。关键数据、分析脚本、中间结果要让助手存成文件留在助手工作目录（即共享研究工作区），不只打印在对话里——便于复现与归档。基于数据给出你的判断，附关键数据/指标。不要只凭记忆表态——要有实证。数据必须可靠：让助手只用权威一手源（官方/国际组织/原始论文），抓取优先照研究 playbook 的已验证配方打（宏观/估值类先查速查表匹配官方端点；无配方的才用聚合器，并在台账登记口径与局限），关键数字两个独立源交叉验证或加总闭合，二手转引只用于定位一手源，分歧与查不到的数据如实标注置信度或缺失，不编造。默认不出图（文本模型看不懂图），除非用户明确要求可视化。」\n`)
  sb.push('   - 角色会各自调助手下数据（可能要几十分钟），engine 收齐所有角色回复后唤醒你。\n')
  sb.push('3. 收齐后，**原样转达**各方研究结论（不要加工），用 action: note 写综合进账本。\n')
  sb.push('\n### 第 2 轮起：交叉迭代（角色看到彼此上一轮结论）\n')
  const round2Data = researchWs !== ''
    ? '数据类深挖先查 DATA_LEDGER.md——管家预取的和各角色助手的专属数据都在台账里（data/core/ 与 data/<角色>/）；复用前先看 来源/口径/抓取时间 三列判断适配性（口径不符的数据再新也是错的），对本次结论起支撑的关键数据让助手 spot-check 源头或对独立源，确认适配才复用；需要新的公共数据仍派管家（它持续待命）。裁决矛盾需独立双源的除外——指定两路角色助手各自独立下取（不复用台账已有文件，拉完各自登记），保独立性，也不是全员重下。新抓取同样优先 playbook 已验证配方。'
    : ''
  sb.push(`4. 再调 action: gather 加 research: true，任务大意：「上一轮各方结论已写入账本，请先读 SYNTHESIS.md 了解其他人的发现，然后补充/反驳/深挖（可再用助手验证）。${round2Data}」任务里点名反驳时先构造最强反例——找出该结论依赖的未验证假设与最薄弱环节，把反例直接带给该角色，不要泛泛说「请反驳」。你自己综合时也做同样自检，再决定下一轮深挖谁。若发起定向数据攻坚（裁决矛盾/补数据缺口），**点名分配**靶点：每个靶点默认 1 个认领者；裁决矛盾类最多 2 个（争议方 + 1 位中立方，独立双源即止），其余角色只做判断、不重下数据。\n`)
  sb.push('5. 收齐后再综合，更新账本。重复直到结束条件触发。\n')
  sb.push('\n**用户中途发言**（在本群插话、追问、给新信息或调整方向）：融入编排——追问用 action: ask 转给相关角色（gather 在途时 ask 会被拒：等收齐或超时唤醒后再问，或把追问并入下一轮 gather 任务），新信息与方向调整并入下一轮 gather 任务；不要无视，也不要中断在途轮次。\n')
  sb.push('\n## 结束条件\n')
  if (mode === 'manual') {
  ...
  const reconStep = researchWs !== ''
    ? `\n  0. **先数字对账，再渲染**：调 feishu_bridge_subtask 工具（action: spawn，worktree: off，dir: ${researchWs}），brief 内容：『读账本目录 ${ledgerDir} 下 SYNTHESIS.md/SUBPROBLEMS.md/RECORD.md，做数字对账：把三份文件里出现的全部关键数字（点位/估值/分位/利率/收益率/天数/阈值/换算值）逐个映射到来源——${researchWs}/DATA_LEDGER.md 台账行、${researchWs}/data/ 数据文件、或脚本/结果产物；可用 ${researchWs}/.venv/bin/python 就地复算，**禁止联网重抓、禁止装包**。文中「X 源核对」类声明要核对声明的源是否真有台账行或抓取产物。结果写 ${ledgerDir}/RECON.md（已验证数字表 + 未命中清单 + 声明核对结果），完成后 action: report，message: 「对账完成：未命中 N 个」。』子任务 report 回父群后：未命中的数字用 action: note 并进综述段——错得明确的直接修正，口径存疑的标「对账存疑」+置信度；对账子任务失败或 ~15 分钟无 report 就跳过、直接进步骤 1 渲染，不卡收尾。对账修正并进账本后再渲染（HTML 读的就是修正后的账本）。`
    : ''
  sb.push(`
...
1. 核心判断直讲（顶部第一屏，默认展开）：直接讲事情本身——这项研究回答了什么问题、核心判断是什么、各方基于数据倾向于什么、在什么地方仍有诚实分歧。2-4 句大白话；把术语展开成大白话（如「交付契约错配」→「交付的定义跟任务对不上」），绕不开的名词就地用一句大白话说明。有定论处给定论，有分歧处显式标出分歧，不要为了干脆而假装分歧已消解。
2. 拆 2-3 个核心发现（默认展开）：把判断拆成 2-3 个「要懂这件事必须跨过的门槛」。每个 = 一句大白话标题 + 关键数据/指标保留数字、含义用大白话讲清意味着什么 + 一个最小实例（用研究里真实的具体事实、数字或场景落地）+ 折叠的专业支撑细节（保真，不降级）。其中【违反直觉】的发现标 ⚠ 反直觉，并多给一句「为什么这件事反直觉 / 常人会怎么误判」。
3. 仍有的分歧（默认展开）：用大白话列出各方仍未达成一致的地方——每条 = 分歧是什么 + 各方分别怎么看 + 这个分歧为什么存在（根本视角差异，不是谁对谁错）。不要假装分歧已被消解。
4. 原始细节（默认折叠）：原始数据、各视角原话贡献、仍未解决的开放问题（明确交回人类定夺）。

原则：信息不丢，数据不丢，只是全部换成大白话——直接讲事情本身、数字保留并用大白话讲清含义、用分歧清单标出边界。完成后调 feishu_bridge_subtask 工具 action: report，message: \"HTML 已生成：<path>\"』。**用子任务是为了隔离 html 渲染，不污染你的 context**；子任务无 worktree、会话日志持久保留，chatroom end 时自动回收。
...
- 用户选「结束并出总结」→ 调 ${TOOL} 工具 action: end + 结构化总结，总结文字同时用 ${TOOL} 工具（action: note 加 section: report）写进账本 REPORT.md 落盘。**若用户此前选过「出一份深度学术版」**，保持学术结构化总结（综合图景、各方贡献与数据、仍有的分歧/开放问题，明确交回人类定夺）；**否则**走白话直讲（数据仍保留：数字不丢，含义用大白话讲清，直接讲事情本身）。不要假装分歧已被消解。
