...
**查学术文献/最新论文**：派任务时点名让助手用 scholar skill（双引擎文献检索 CLI，其 catalog 里有；skill 内置 SerpAPI 用量规则，load 后照办），不要拿普通 web search 硬搜学术库。
...
- 自由使用你的技能和工具来支撑论点——查学术文献/最新论文用 scholar skill，其他时效信息用 web search。
...
- 用你自己的视角、用大白话回答，要深度不要注水。自由使用你的技能和工具来支撑论点——查学术文献/最新论文用 scholar skill，其他时效信息用 web search。
...
- （可选）REPORT.md — 上次收尾时落盘的结论文本
- （可选）「前情」区 — 指向被延续的上一个聊天室的指针，**未经本次讨论验证**：那只是指针，前次判断采信与否由主持人甄别后写进综述段——你以综述段为准，别把前情当已验证事实。
RECORD.md 只保留近期记录，更早轮次归档在同目录 RECORD-*.md，需要回溯再读。
回答前把文件都 Read：别只靠被问到的那个问题——查账本，让你的回答建立在完整图景上，而不是重复或与已有内容相矛盾。
`
}
