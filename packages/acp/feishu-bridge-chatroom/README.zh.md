...
- **闪电轮只在引擎内存且边缘靠提示词纪律**：poll barrier 从不持久化（跟踪的一次性查询随进程消亡，重启恢复无物可续——主持人重新发起该轮）；收尾补盲只在主持人遵循 priming 指令时发生（无 `end` 时刻强制）；每条表态在配置的 `pollProvider` 路由或默认路由上跑完整 persona 会话——会话 origin `oneshot` 关掉 LLM 标题生成，由 adapter 重新注册角色目录的记忆索引注入，人设连续性不再花费标题调用。表态唤醒与 gather 摘要同等有界；并发查询以 `pollMaxConcurrent`（默认 4）为帽，防止 13 人的库一次性扇出网关。
...
- **被放弃的引导流程会留下开场快答出席行**：开场闪电轮先于 `start` 消费场次号就把逐角色 `RECORD.md` 行落进下一场次目录；引导挑选流程随后被放弃（用户取消，或重启后的孤儿卡——watchdog 提示卡只是建议、并不中止）时这些行不会被删除，同一 hub 的下一个聊天室开账时账本里就带着被放弃那轮的表态。删除它们需要一份持久的逐 hub 台账行登记，加上散布在各个 picker 取消路径的清理钩子（而且只覆盖取消仍会漏掉「结算后、放弃前重启」的场景）；在泄漏再次造成真实困扰之前暂缓。
