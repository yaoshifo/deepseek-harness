...
| `replayModelIdentity` | `resolved` | Which model id replayed assistant messages carry: `requested` stamps the route's requested id on `anthropic-messages` history, for gateways whose reported model name is another spelling of the same route model, so pi-ai replays thinking blocks as same-model instead of converting them to plain text |
...
Successful assistant responses store a versioned, lossless-JSON replay state beside the provider and model that produced them — response-level facts plus one per-block entry per streamed block. At request time, `LlmRuntime` passes replay state only when the same adapter instance owns both routes; the adapter validates it and restores native response ids, provider signatures, and optional `providerThinkingLevel` effort metadata, keeping absent effort metadata absent. Replay validates the requested model identity against the assistant source and separately restores an Anthropic response model when the provider resolved an alias or fallback; a route setting `replayModelIdentity: requested` opts out of that resolved-model stamp, because a gateway whose reported name merely respells the route id must not make pi-ai treat its own history as cross-model — cross-model replay converts every thinking block to plain text, and a permanent name mismatch would push the model's reasoning into the text channel turn after turn. An unusable state degrades to provider-neutral content instead of failing the request. pi-ai tool-call arguments are parsed objects, so the adapter parses input and re-stringifies output to the harness raw-JSON convention; pi-ai in-stream error events map to terminal `finish` chunks.
