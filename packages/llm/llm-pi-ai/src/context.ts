...
export function tagStrictSampling(context: PiContext, model: Model<Api>): void {
  ...
  const strictTools = compat !== undefined && 'supportsStrictTools' in compat && compat.supportsStrictTools
  const strictMode = compat !== undefined && 'supportsStrictMode' in compat && compat.supportsStrictMode
  if (!strictTools && !strictMode) return
