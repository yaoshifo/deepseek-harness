...
| 工具 | 参数 | 行为 |
|---|---|---|
| `glob` | `pattern`、`path?` | 查找路径匹配 glob 模式的文件，包含隐藏与忽略文件但排除 VCS 元数据；不含 `/` 的模式匹配任意深度的基名，因此 `*` 匹配整棵树；模式锚定在搜索根（`path` 参数，缺省为会话工作区）——`~/` 前缀与绝对路径模式先按同样方式解析，搜索根之外的绝对模式会被拒绝；完整结果保持按修改时间排序 |
| `grep` | `pattern`、`path?`、`include?` | 用 ripgrep 正则搜索文件内容，并按文件分组返回 `Line N: <preview>` 匹配；`include` 是一个相对搜索根匹配的正向 glob 过滤器，逗号分隔列表与否定值会被前置拒绝 |
