 ...
 * rejection carries the file's current read window (see `unread-attachment.ts`).
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: EditToolArgs, exec) {
      ...
      try {
        ...
        // (shared with write via unread-attachment).
        ...
        throw remediated instanceof FsError && remediated.code === 'FS_NOT_OBSERVED'
          ? await enrichNotObserved(ctx, exec, target, caps, remediated, 'edit')
          : remediated
      }
