...
| `envFile` | 无 | `KEY=VALUE` 文档，其条目以原始名合并进每条命令的环境 |
...
环境中的凭据形状变量名（`*KEY*`、`*PASSWORD*`、`*SECRET*`、`*TOKEN*`）在到达命令前会被擦除——这是 seam 的既定规则，harness 侧的 secret 不会隐式泄漏。`envFile` 是部署围绕该规则的显式选择：指向一个由操作者维护的 `KEY=VALUE` 文档（通常 `chmod 600`），每条命令——前台与后台——都会以原始名携带这些条目运行，分层在调用方显式条目与受信 `DSH_*` 事实之下、终端覆盖之上。文件在每次 spawn 时重读：改值、追加键或删除行都在下一条命令生效，无需重启。配置的文件缺失或格式错误会使插件加载失败并指明行号；执行器运行期间文件消失时，该条命令响亮失败而不是静默丢弃条目。条目对命令可见，因此对模型可见——把该文件当作「模型运行的命令允许持有什么」的评审白名单，而不是藏匿处。

当组合了 system-prompt 服务且配置了 `envFile` 时，执行器注册一个 boot 固定的提示小节（`bash:env-file`），告知模型这些条目是逐命令的受信注入——在命令环境中存在、在 daemon 进程环境中不存在属于预期行为而非值得探究的异常——且永不回显其值。当前的键名另行搭载在 `DSH_ENVFILE_KEYS` 标记变量里，与值一样每次 spawn 重建，因此追加的键在下一条命令即出现在标记中（小节文本 boot 固定；settings 中变更 `envFile` 需重载才会移动小节）。
...
`bash:env-file` 提示小节（仅在配置了 `envFile` 时存在）是 boot 固定的请求前缀内容：只增加一个稳定小节、请求之间永不变化，因此除首个被缓存的请求外不再使任何缓存失效。其余部分不会直接导致 KV Cache 失效；请求前缀的任何变更由具名消费方负责。
