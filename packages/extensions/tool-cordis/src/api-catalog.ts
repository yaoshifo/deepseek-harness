...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    summary: 'Registry of scoped suppression markers for workspace-instruction injection.',
    description: 'Registry of scoped suppression markers for workspace-instruction injection. Registering on a service (rather than on the plugin body) exposes the scoped suppress seam: consumers composing a wholesale-replacement persona call it in their agent setup so the instruction channel stays silent for that agent.',
    methods: [
      {
      ...
      {
        signature: 'suppressedFor(agent: Agent): boolean',
        description: 'Whether any suppression marker covers this agent: the global layer or any layer along the agent\'s scope chain.',
        parameters: [{ name: 'agent', description: 'the agent whose scope chain is inspected.' }],
  ...
  },
  {
    ...
    methods: [
      {
        signature: 'denyTools(engine: Engine, names: readonly string[]): () => void',
        description: 'Register tool names hidden from every session of one engine\'s project.\n\nA sibling plugin calls this for a project it is disabled on (the chatroom plugin hides `feishu_bridge_chatroom` on projects configured `enabled: false`), so the tool definition stops entering that project\'s model requests instead of merely failing at execute.',
        parameters: [{ name: 'engine', description: 'The engine whose project sessions the mask applies to.' }, { name: 'names', description: 'Tool names to hide; names absent from the live registry are dropped by the adapter\'s mask, not here.' }],
        returns: 'Disposer removing the names again; idempotent.',
      },
      {
        signature: 'deniedToolsOf(engine: Engine): readonly string[]',
        description: 'The tool names currently registered as hidden for one engine\'s project.',
  ...
  {
    key: 'mcpWorkspace',
    summary: 'The directory MCP discovery service.',
    description: 'The directory MCP discovery service.\n\n`wrap(setup)` composes the directory mount onto a creation-time AgentSetup (fresh, fork, and resume paths alike): the inner setup runs first so its publication commit propagates, then every `.mcp.json` server mounts into the agent scope through `agentCtx.plugin(mcpClient, config)`. `mountedFor` answers which servers a cwd would mount, for `/mcp` display.',
    methods: [
      {
        signature: 'wrap(setup: AgentSetup | undefined): AgentSetup',
        description: 'Wrap a creation-time AgentSetup with this session\'s directory MCP mount. The inner setup runs first (a project tool mask computed inside it must not see the directory tools), then `.mcp.json` servers mount into the agent\'s own scope and disappear with it. Each server failure degrades to that server\'s tools being absent; session creation never rolls back.',
        parameters: [{ name: 'setup', description: 'the inner creation-time setup, or undefined.' }],
        returns: 'the wrapping setup.',
      },
      {
        signature: 'async mount(agentCtx: Context, agent: Agent): Promise<void>',
        description: 'Mount the session-cwd `.mcp.json` servers into one agent scope. The composition primitive behind wrap, for callers that already hold an agent creation window of their own.',
        parameters: [{ name: 'agentCtx', description: 'the unpublished agent\'s scoped creation context.' }, { name: 'agent', description: 'the unpublished Agent whose session cwd drives discovery.' }],
      },
      {
        signature: 'async mountedFor(cwd: string): Promise<readonly DirectoryMcpServer[]>',
        description: 'List the servers a directory mount would load for one cwd, for `/mcp`.',
        parameters: [{ name: 'cwd', description: 'the session working directory to inspect.' }],
        returns: 'mapped server entries; empty when the cwd is untrusted, the file is absent, or parsing fails.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Compose one deployment capability into every continuable child\'s unpublished creation context on fresh creation and cold resume. Awaitable installs settle inside the child\'s creation window, before the next contribution and before publication. Grants wait for the next Activation; removing the contribution revokes every resident installation immediately.',
        parameters: [{ name: 'contribution', description: 'child-scope installer, synchronous or awaitable.' }],
        returns: 'the exact Cordis effect disposer.',
      },
      {
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    mode: 'waterfall',
    signature: '\'feishuBridge/route-human-reply\'(payload: { engine: Engine; platform: Platform; sessionKey: string; content: string; machine: boolean }, next: () => boolean): boolean',
    ...
    description: 'Route the human\'s reply to a feature\'s pending question: a listener that consumes the message short-circuits with `true` and the inbound flow stops there — this decision outranks command dispatch and permission handling. The built-in base returns false (no feature holds a pending question). Machine messages (deliverMachineMessage wakes) are never human replies — listeners must skip them.',
    parameters: [{ name: 'payload', description: '.machine - True when a synthetic machine message, never a human reply.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'CardChart',
    declaration: 'export interface CardChart {\n    kind: \'chart\';\n    spec: VChartSpec;\n    aspectRatio?: \'1:1\' | \'2:1\' | \'4:3\' | \'16:9\';\n}',
  },
  {
  ...
  {
    name: 'CardElement',
    declaration: 'export type CardElement = CardMarkdown | CardDivider | CardActions | CardNote | CardListItem | CardSelect | CardCheckOptions | CardImage | CardChart | CardCollapsiblePanel | CardForm | CardInput | CardColumnSet;',
  },
  {
    name: 'CardImage',
  ...
  },
  {
    ...
    declaration: 'export type ContinuableSetupContribution = (childCtx: Context, child: Agent) => (() => void) | Promise<(() => void) | void>;',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export type DeliveryOutcome = \'sent\' | \'failed\' | \'unknown\';',
  },
  {
    name: 'DiffCallView',
  ...
  {
    name: 'DirectoryMcpServer',
    declaration: 'export interface DirectoryMcpServer {\n    readonly name: string;\n    readonly transport: \'stdio\' | \'streamable-http\';\n}',
  },
  {
  ...
  {
    name: 'MonitorPollPage',
    declaration: 'export interface MonitorPollPage {\n    messages: Message[];\n    latestTimeSec: number;\n}',
  },
  {
  ...
  {
    name: 'ParkOutcome',
    declaration: 'export type ParkOutcome = \'approved\' | \'rejected\' | \'answered\' | \'cancelled\';',
  },
  {
    name: 'PendingAsk',
  ...
  {
    name: 'PlanLayers',
    declaration: 'export interface PlanLayers {\n    plain: string;\n    details: string;\n}',
  },
  {
    name: 'Platform',
  ...
  {
    name: 'ProgressStatus',
    declaration: 'export interface ProgressStatus {\n    state: \'running\' | \'completed\' | \'failed\' | \'thinking\' | \'waiting\' | \'approved\' | \'rejected\' | \'answered\' | \'cancelled\' | \'truncated\';\n    ts: string;\n    toolCallSeq: number;\n    pendingSubtasks?: number;\n}',
  },
  {
  ...
  {
    name: 'UserMeta',
    declaration: 'export interface UserMeta {\n    userName: string;\n    chatName: string;\n}',
  },
  {
    name: 'UserQuestion',
  ...
  {
    name: 'VChartSpec',
    declaration: 'export type VChartSpec = Record<string, unknown>;',
  },
  {
  ...
  {
    name: 'WorktreeMode',
    declaration: 'export enum WorktreeMode {\n    ForceOff = 0,\n    Auto = 1,\n    ForceOn = 2\n}',
  },
]
