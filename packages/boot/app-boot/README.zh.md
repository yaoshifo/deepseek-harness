...
带 `patchReload: live` 的 profile 会监视两份用户 patch 文件：有效编辑无需重启即可重新组合，被拒绝的编辑则让最后一个可用应用继续运行。`startup` profile 既不安装这些监视器，也不安装 launcher 的仅监视 HMR 回退。部署也可以为 live profile 冻结监视：`DSH_CONFIG_HMR_DISABLED`（任意非空值，与 telemetry 开关同语义）在 launcher 层豁免——不挂载回退 HMR 服务、不注册用户 patch 监视器，patch 层编辑只在下次启动时生效。
...
- [用户 patch HMR 测试](../../../.agents/notes/implemented/testing/2026-09-09-user-patch-hmr-test-delivery.zh.md)——事务行为与原生文件系统交付的归属。
