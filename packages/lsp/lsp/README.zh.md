...
`dsh-lsp` 为 harness 提供语言服务器代码导航：agent 可以转到符号的定义、查找其引用、跳转到其实现或阅读悬停文档，代码导航服务（`ctx.lsp`）会把每个查询路由到拥有该文件扩展名的语言服务器提供方。提供方按品牌化 id 与文件扩展名注册，因此更换提供方绝不会改变请求导航的方式，也不会改变模型看到的内容。该服务恰好暴露四个位置操作外加按名的工作区符号查找，全部只读且没有通用 JSON-RPC 逃生口；它自身不贡献提示词或工具 schema——面向模型的 `lsp` 工具位于 `dsh-tool-lsp`。与 `dsh-lsp-stdio` 之类的提供方及该工具组合，即可为 agent 提供精确导航；本包单独加载时什么也不做。
...
当部署希望模型可见的代码导航由语言服务器支撑时，选择此服务。它覆盖只读导航——定义、引用、实现与悬停——并刻意排除修改（重命名、code action、格式化）、`documentSymbol` 列表查询与诊断。该服务提供方无关：本地 stdio 服务器、远程服务器与沙箱原生提供方都以相同方式注册，因此更换后端不会改变模型看到的内容或请求方式。
...
`LspSymbolRequest`（`query`、`workspaceRoot`、可选 `seedFilePath`）命名该 seam 的符号查找：没有文件、没有位置，匹配语义属于 server。带种子的查找与 `query()` 一样按种子扩展名路由；无种查找扇出。每个提供方回答一个 `LspSymbolResult`（按其 server 的相关性顺序排列的 `symbols`，加上各自的 `resolvedWorkspaceUri`）；未解析的 `WorkspaceSymbol` 以 `location: null` 保留条目。`seedFilePath` 指定一个临时打开的项目文件，供符号索引依赖已加载项目（tsserver）的服务器使用。`LspSymbolsMerged` 携带各组结果，以及 `failures`（其他提供方成功时失败的那些）或 `uncoveredSeedExtension`（没有提供方覆盖种子语言，未做任何查询）。完整约定见 `src/types.ts`；`src/index.ts` 给出 `LspError` code，包括 `LSP_DISPOSED` 和 `LSP_MALFORMED_RESPONSE`。
...
- **四种位置操作加按名符号查找**——call hierarchy 与 `documentSymbol` 暂缓（需要不同 schema）；diagnostics 需要独立的新鲜度与累积规则；修改（重命名、code action、格式化）需要独立工具，并集成预览、权限与写入策略。
- **对上游 seam 的 fork 扩展**——上游把该 seam 封闭在四种操作；`symbol()` 是 fork 本地新增，等待上游出现等价能力（若出现则做语义合并）。
- **没有观测表层**——可用性只能通过运行 `query()` 或 `symbol()` 并按抛出的 `LspError` code 路由来观测；没有提供方变更事件或能力状态查询。
