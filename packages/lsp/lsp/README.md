...
The **LSP capability seam**: an abstract `LspService` (`ctx.lsp`) defining WHAT semantic code navigation the harness has — go to definition, find references, find implementations, hover, and workspace symbol lookup by name — over language-server providers, without binding the model contract to local subprocesses.
...
| `symbol(request, signal?)` | Fan one name-based symbol lookup out to every registered provider in registration order (a symbol query has no file extension to route on) and merge their groups. A provider whose server lacks `workspaceSymbolProvider` contributes nothing; every provider lacking it throws `LspError` `LSP_UNSUPPORTED_OPERATION`, and no provider registered throws `LSP_UNAVAILABLE`. An optional `seedFilePath` opens a project file transiently for servers that index symbols only per loaded project. |
...
`LspSymbolRequest` (`query`, `workspaceRoot`, optional `seedFilePath`) names the seam's symbol lookup: no file, no position, and server-side matching semantics. Each provider answers an `LspSymbolResult` (`symbols` in its server's relevance order, plus its own `resolvedWorkspaceUri`); an unresolved `WorkspaceSymbol` keeps its entry with `location: null`. `seedFilePath` names a project file to open transiently for servers (tsserver) whose symbol index requires a loaded project. See `src/types.ts` for the full contracts and `src/index.ts` for the `LspError` codes, including `LSP_DISPOSED` and `LSP_MALFORMED_RESPONSE`.
