...
describe('lsp-stdio end to end over a fake server', () => {
  ...
  it('routes a seeded symbol query to the covering server only', async () => {
    await writeFile(join(ws, 'a.py'), 'x = 1\n')
    const ctx = new Context()
    await ctx.plugin(Lsp)
    await ctx.plugin(LocalSubprocessRuntime)
    ...
    await ctx.plugin(LspLocal, {
      servers: {
        typescript: fakeServer({
          LSP_FAKE_CAPS: JSON.stringify({ workspaceSymbolProvider: true }),
          LSP_FAKE_SYMBOLS: JSON.stringify([{ name: 'tsOnly', kind: 13, location: locationJson(0) }]),
        }),
        python: fakeServer(
          {
            LSP_FAKE_CAPS: JSON.stringify({ workspaceSymbolProvider: true }),
            LSP_FAKE_SYMBOLS: JSON.stringify([{ name: 'pyOnly', kind: 13, location: locationJson(0) }]),
          },
          { extensionToLanguage: { '.py': 'python' } },
        ),
      },
    })
    // The .py seed routes to the python server: its symbols answer, the typescript one stays cold.
    const routed = await ctx.lsp.symbol({ query: 'Only', workspaceRoot: ws, seedFilePath: 'a.py' })
    expect(routed.groups).toHaveLength(1)
    expect(routed.groups[0]?.symbols[0]?.name).toBe('pyOnly')
    await ctx.fiber.dispose()
  })

  it('reports an uncovered seed extension without starting any server', async () => {
    const ctx = await mount({
      LSP_FAKE_CAPS: JSON.stringify({ workspaceSymbolProvider: true }),
      LSP_FAKE_SYMBOLS: JSON.stringify([{ name: 'x', kind: 13, location: locationJson(0) }]),
    })
    // No .ml route exists; nothing is queried and the extension comes back for the hint.
    expect(await ctx.lsp.symbol({ query: 'x', workspaceRoot: ws, seedFilePath: 'notes.ml' }))
      .toEqual({ groups: [], uncoveredSeedExtension: '.ml' })
    await ctx.fiber.dispose()
  })

  it('retains one server failure alongside another server\'s groups in a seedless fan-out', async () => {
    const ctx = new Context()
    await ctx.plugin(Lsp)
    await ctx.plugin(LocalSubprocessRuntime)
    await ctx.plugin(LocalFileSystem, { cwd: process.cwd() })
  ...
  it('reads and opens a seed document for a symbol query', async () => {
    ...
    expect((await ctx.lsp.symbol({ query: 'x', workspaceRoot: ws, seedFilePath: 'a.ts' })).groups).toHaveLength(1)
    await ctx.fiber.dispose()
  })
