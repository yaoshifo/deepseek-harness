...
- **tsserver needs a loaded project for symbols** — `navto` answers `No Project` until a document opens and the project unloads when the last document closes, so a cold seedless symbol query surfaces that server error; the seed mechanism and the remembered-document fallback cover the warm and seeded paths, and the pinned e2e records the cold failure. On large composite graphs the configured project also takes seconds to load after the first open: until it finishes, tsserver serves single-file inferred-project results (narrow `navto` and reference scope); once loaded the project stays for the pooled instance's lifetime, so only a server's first queries are narrow. A seedless fan-out in a mixed-language deployment surfaces this as a per-provider failure note rather than sinking the other providers' answers (the seam merges); other servers (pyright, gopls, rust-analyzer) index the whole workspace at startup and need no seed.
