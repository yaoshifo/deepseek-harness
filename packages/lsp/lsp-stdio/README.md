...
- Runs **symbol queries with an optional seed document**: a `symbol()` request canonicalizes the workspace, then sends `workspace/symbol { query }` through the same serialization queue and transport-replacement policy. The request's optional `seedFilePath` is read and derived a language id from this provider's mapping, and the instance opens it transiently around the request; without a seed the instance re-opens the last document a position query opened. Servers like tsserver answer a seedless, never-opened symbol query with `No Project`; a server without the `workspaceSymbolProvider` capability fails the provider call as `LSP_UNSUPPORTED_OPERATION`, which the seam folds into an empty contribution.
