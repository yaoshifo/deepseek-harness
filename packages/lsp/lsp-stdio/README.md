...
- Runs **symbol queries with an optional seed document**: a `symbol()` request canonicalizes the workspace, then sends `workspace/symbol { query }` through the same serialization queue and transport-replacement policy. The request's optional `seedFilePath` is read and derives a language id from this provider's mapping (a seed whose extension this provider does not handle seeds nothing here), and the instance opens it transiently around the request, remembering it for later seedless queries; without a seed the instance re-opens the last document it opened. Servers like tsserver answer a seedless, never-opened symbol query with `No Project`; a server without the `workspaceSymbolProvider` capability fails the provider call as `LSP_UNSUPPORTED_OPERATION`, which the seam folds away.
...
- **Transient-open compatibility floor** — servers whose synchronization omits open/close (or advertises `None`) are unsupported even if closed-document queries would work; the pinned TypeScript e2e establishes one compatibility floor, not a cross-language claim. A seedless symbol query skips the document lifecycle, so a sync-less server can still serve it if it advertises `workspaceSymbolProvider`.
- **UTF-16 position encoding only** — a server negotiating any position encoding other than `utf-16` is rejected wholesale at initialization; the same kind of compatibility floor as transient open.
- **tsserver needs a loaded project for symbols** — `navto` answers `No Project` until a document opens, so a cold seedless symbol query surfaces that server error; the seed mechanism and the remembered-document fallback cover the warm and seeded paths. On large composite graphs the configured project also takes seconds to load after the first open: until it finishes, tsserver serves single-file inferred-project results; once loaded the project stays for the pooled instance's lifetime. A seedless fan-out in a mixed-language deployment surfaces this as a per-provider failure note rather than sinking the other providers' answers.
- **No `workspace/symbol/resolve`** — unresolved `WorkspaceSymbol` entries stay `location: null` rather than making a resolve round-trip; servers in current use return resolved locations.
