...
`lsp` 接受 `operation`（`workspaceSymbol`、`goToDefinition`、`findReferences`、`goToImplementation` 或 `hover`）。`workspaceSymbol` 接受 `query`（非空符号名；不需要坐标）与推荐的、符号同语言的 `file_path` 种子文档——查询路由到该语言的服务器，种子语言没有被任何已配置服务器覆盖时返回安装建议而不是结果。其余四种操作接受 `file_path`、`line` 与 `character`，其中 `line` 与 `character` 是正的、从 1 开始的 UTF-16 光标坐标；工具将其转换为 seam 从零开始的位置。`findReferences` 始终包含声明，因此影响分析绝不会遗漏定义位置。提供方、language id、工作区根目录、限制、超时与可执行文件均不进入模型输入。
...
| 键 | 默认值 | 含义 |
|---|---|---|
| `maxLocations` | `100` | 出现省略标记前可渲染位置或符号的最大数量。 |
...
一个系统提示词区段（顺序 112）将按名查找定位为入口、将位置操作定位为精确后续步骤，文本如下：
...
每项工具结果以 `maxResultChars` 为上限，`maxLocations` 还会限制导航与符号项数量。
...
无。客户端渲染通用搜索卡片：`{ card: 'generic', kind: 'search', title, locations: [{ path, line }] }`；从 args 派生的标题携带操作与从 1 开始的光标，跟随焦点对准查询行，标题则保留列号。`workspaceSymbol` 调用没有光标，标题携带带引号的查询词且不带位置。
