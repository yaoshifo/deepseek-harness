...
describe('ToolRuntime', () => {
  ...
  describe('ask routing through ctx.approval', () => {
    ...
    it('grants on allowed-always and skips re-asking the same tool for the agent', async () => {
      ...
      const first = await ctx.tools.execute({ signal: testToolSignal, callId: ToolCallId('c1'), name: 'echo', arguments: { text: 'a' }, agent })
      const second = await ctx.tools.execute({ signal: testToolSignal, callId: ToolCallId('c2'), name: 'echo', arguments: { text: 'b' }, agent })
    ...
    it('forwards a bounded toolInput preview to the answerer', async () => {
      ...
      await ctx.tools.execute({ signal: testToolSignal, callId: ToolCallId('c1'), name: 'echo', arguments: { text: 'hi' }, agent: fakeAgent() })
...
describe('input-boundary key-style normalization', () => {
  it('executes a defineTool call whose arguments name parameters in the opposite key style', async () => {
    ...
    ctx.tools.register(defineTool({
      ...
      output: { schema: { type: 'boolean' }, render: (_args, value) => [{ type: 'text', text: value ? 'true' : 'false' }] },
      async execute(args) {
        seen = args
