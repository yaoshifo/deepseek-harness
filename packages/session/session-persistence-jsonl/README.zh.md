...
`readOnlyRoots` 把另一个进程的会话存储（例如写自己根目录的 bridge 守护进程）挂载进本进程，用于列出与冷读取。所有写路径仍然只指向 `root`：`create` 拒绝已存在于任一根下的 id；对存储在只读根下的会话发起写打开会报错并指名两个根。读取按与 `root` 相同的最高代与迁移规则跨所有根解析会话 id；同一会话 id 出现在多个根下会大声失败而不是任选一份。每个配置的只读根必须是已存在的可读目录、不得重复、且不得与 `root` 相同——否则加载即拒绝，因为本后端从不创建它们。
