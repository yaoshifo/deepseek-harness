...
A second **Profiles** tab in the same section calls `ctx.remote.pluginInventory.listProfiles()` and renders one collapsed disclosure card per stored harness profile: the header carries the profile name and its bundle-stack summary, and the disclosure lists the profile directory, the bundle stack, the flattened patch-entry rows with their enablement tags, and the raw `cordis.yml` / `cordis.patch.yml` composition files read-only — the console view for inspecting how another deployment's profile (for example a bridge daemon's) is composed.
