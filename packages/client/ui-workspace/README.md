...
Once the Workspace list baseline and the session list are both ready, browser-persisted expansion and Session-order records retain only current Workspace ids plus Ungrouped, the flat-list account, and the cwd-derived directory-group keys of existing unaccounted Sessions; a directory key with no unaccounted session left is pruned. Real Workspaces initialize from `WorkspaceView.sessionIds`, while Ungrouped, the cross-Workspace flat list, and every cwd-derived directory group initialize from recency. The shared sidebar projection hides rows whose durable Session summary has `origin: 'subagent'`, and each visible ordinary row inherits the blue activity indicator while any descendant reached through uninterrupted subagent-origin lineage is running. The same pure derivation reads the Schedule key from list projection values for grouped, flat, and search nodes; the package uses only the type-only `@deepseek-ai/dsh-schedule/client` dependency and does not import the Schedule runtime or `ui-schedule`.
