 ...
 * mount it as a no-op. The plugin stays serviceless so preset rows can mount it
 * without publishing a process-global service; scoped suppression lives in the
 * host-plane `@deepseek-ai/dsh-agent-instructions/suppression` service, which the
 * plugin reads optionally — a missing registry suppresses nothing.
 *
...
/**
 * Plugin body: workspace-instruction injection with the validated config. The
 * optional host-plane suppression registry (`agentInstructionSuppression`)
 * decides per agent whether any instruction is composed at all.
...
export function apply(ctx: Context, config: Config): void {
  ...
  const compose = async (
    ...
    if (ctx.get('agentInstructionSuppression')?.suppressedFor(agent)) return undefined
