 ...
 * mount it as a no-op. The mounted `agentInstructions` service exposes scoped
 * suppression ({@link AgentInstructions.suppress}) for compositions that replace the
 * persona wholesale: a suppressed scope receives no baseline and no dynamic updates.
 *
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    /** Workspace-instruction injection and its scoped suppression. */
...
/** One scope's aggregate suppression markers for {@link AgentInstructions.suppress}. */
...
/**
 * Service owning workspace-instruction injection. Registering the listeners on a
 * service (rather than directly on the plugin body) is what exposes the scoped
 ...
 */
export class AgentInstructions extends Service {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    const compose = async (
      ...
      if (this.suppressedFor(agent)) return undefined
  ...
  suppress(): () => void {
    return this.suppressions.effect(
      ...
      { label: 'agentInstructions.suppress()', notify: false },
    )
  }
   ...
   */
  private suppressedFor(agent: Agent): boolean {
...
/**
 * Plugin body: mount the {@link AgentInstructions} service with the validated
 * config, so injection and the scoped suppression seam share one owner.
