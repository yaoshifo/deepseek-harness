...
describe('workspace instruction imports', () => {
  ...
  it('refreshes the parent scope when an imported file changes and is touched', async () => {
    ...
    try {
      ...
      expect(blocksText(((await syncedAgentInstructions(ctx, agent))).content)).toContain('new shared rule with more detail')
    } finally {
      await ctx.fiber.dispose()
      await rm(root, { recursive: true, force: true })
      await rm(home, { recursive: true, force: true })
...
describe('dynamic nested workspace context injection', () => {
  ...
  it('promotes the next existing sibling in a nested scope when the preferred candidate is deleted under first-existing', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: ToolCallId('read-nested-preferred-candidate'),
        name: 'read',
        arguments: { file_path: join('pkg', 'deep', 'file.txt') },
        agent,
      })

      const initial = blocksText((await syncedAgentInstructions(ctx, agent)).content)
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: ToolCallId('read-after-preferred-deleted'),
        name: 'read',
        arguments: { file_path: join('pkg', 'deep', 'file.txt') },
        agent,
      })
  ...
  it('suppresses a previously loaded sibling in a nested scope when a preferred candidate is created under first-existing', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: ToolCallId('read-nested-fallback-candidate'),
        name: 'read',
        arguments: { file_path: join('pkg', 'deep', 'file.txt') },
        agent,
      })
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: ToolCallId('read-after-preferred-created'),
        name: 'read',
        arguments: { file_path: join('pkg', 'deep', 'file.txt') },
        agent,
      })

      const suppression = await syncedAgentInstructions(ctx, agent)
