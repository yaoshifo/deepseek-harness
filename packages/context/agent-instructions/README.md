...
Any candidate file may reference other files with `@path/to/file`; the referenced content is imported into the instruction where the reference stands, between `Imported from:` and `End imported from:` marker lines. Relative paths resolve against the directory of the file containing the reference, `~/`-prefixed paths resolve against the operating-system home directory, and absolute paths load whatever the mounted filesystem provider allows. References inside inline code spans or fenced code blocks stay literal, so `` `@README` `` mentions a path without importing it. Imported files may import further files, up to four hops; a reference whose file cannot be loaded or exceeds that depth renders one `[instruction import unavailable: <path>]` line. Each imported file is read under `maxSourceBytes` and its content counts against `maxBytes` like any other instruction text. Touching an imported file with `read`, `write`, or `edit` refreshes the referencing file's scope on the next request, exactly as editing the referencing file itself would.
...
`candidateSelection` selects the per-directory rule for both lists and defaults to `'all-existing'`, the load-everything behavior above. With `'first-existing'`, each directory contributes at most one file per list: the earliest existing candidate in configured order loads and later siblings stay suppressed regardless of content — with `['CLAUDE.md', 'AGENTS.md']`, a directory holding both files renders only `CLAUDE.md`, while an AGENTS.md-only directory still renders `AGENTS.md`. Reconciliation follows the same rule: deleting a preferred candidate promotes the next existing sibling, and creating a preferred candidate removes a previously loaded sibling. The user-global file never participates in candidate selection.
...
| [`src/imports.ts`](src/imports.ts) | `@path` reference parsing and recursive import expansion |
...
- **Candidate semantics stay intentionally small** — lowercase names and `.claude/rules/` are not interpreted; project scopes load `AGENTS.local.md`/`CLAUDE.local.md` overlays by default, but the user-global `$DSH_HOME` scope has no local overlay and other custom names require explicit candidate configuration.
- **Imports keep fixed Claude Code parity limits** — recursion stops at four hops, repeated references to the same file expand separately wherever they appear, only `/`-separated paths are supported, and there is no interactive approval for imports resolving outside the working directory; the filesystem provider policy is the confinement mechanism.
...
- **Symlinked instruction files and absolute-path imports cross the trust boundary** — a candidate whose final component is a symlink is resolved and its target loaded, and an `@path` import with an absolute or `~/`-prefixed path reads whatever the mounted filesystem provider allows, so a cloned repository can surface off-tree file content as lower-authority workspace guidance (it never overrides system, developer, or direct user instructions). Confine `ctx.fs` with the filesystem policy gate or an OS sandbox when loading untrusted repositories.
...
- **Scoped suppression does not survive an in-process hot remount** — markers live only in the mounted service instance; a remount drops every live agent's marker (its disposer becomes a no-op) and injection resumes mid-session. Restarts that re-run agent setup (config HMR disabled, the feishu-bridge default) recover naturally.
