...
- **作用域限制。** 带作用域的 context（agent 或 preset 组合）可用 `ctx.skills.restrict({ allow?, deny? })` 掩蔽继承的 skill：查看链上所有层的限制对继承名称求交，而该作用域自身的注册不受过滤。不带作用域的调用会抛错；名称只按语法校验，因为可用性随 cwd 变化——在某个工作目录下不匹配任何 skill 的被拒名称在该处是惰性的。
...
读取（`list`/`snapshot`）会收集每一层的候选项：先是运行时 skill，再是各提供方的 `list()` 结果，提供方依次等待、失败被包含。候选项经验证后在层内去重，跨层合并——全局层与祖先层构成继承面，受查看链上的限制约束；该作用域自身的层最后覆盖且在过滤之外；摘要按名称排序。完成的收集按 cwd、scope 链与 revision 缓存，上限为 `collectCacheMaxEntries`；读取中途提供方或运行时变更使 revision 递增时，进行中的收集会重试一次，第二次变更则返回最新候选项并标记为不完整、不予缓存。
...
注册表没有 TTL：只有提供方调用其注册作用域内的 `invalidate()`、发生运行时或限制的注册与释放、或提供方注册变更时，才会清除已完成的目录。每次失效都会递增 revision、清空缓存，并发出不带过滤条件的 `skills/change` 事件；消费方用各自的查找选项重新获取。`invalidate()` 仅当接收它的那条精确注册仍处于活动状态时才生效，因此延迟回调无法干扰同名替代提供方。
