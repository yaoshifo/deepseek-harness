...
Invalid or stale selections return exactly `Error: invalid skill name "<name>"`, `Error: skill "<name>" is unknown or no longer available; available skills include: <a>, <b>, <c> (<N> total) — use the closest name` (up to three model-invocable names whose lowercase form contains the request, else the alphabetical first three; `...; no skills are available in this session` when none exist), or `Error: skill "<name>" is not available for model invocation`. Provider-thrown lookup text is data-dependent and receives the same `Error: <message>` wrapper.
