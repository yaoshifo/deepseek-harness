...
The manager derives three internal residency conditions from Agent quiescence, the owned-child set, and live background jobs rather than maintaining a second state machine: running (an active admission, open turn, or waking inbox work), waiting (quiescent but still owning at least one undisposed child or one live background job), and settled (quiescent with every owned child disposed and no live background job, so the manager disposes the `AgentHandle` and removes the Activation). Live jobs count like undisposed children because the background-job tool contract promises a completion wake, and handle disposal cancels the job before that wake can land — silently stranding the work the child scheduled its plan around. Job settlements re-observe residency through the same wake the ownership graph uses, so a child held only by jobs settles once its last job reaches a terminal record. Forced teardown is never gated on job residency: draining a lineage still cancels agents first, and agent disposal cancels their owned jobs through the registry. Every continuation message uses `Agent.followup()` and becomes one FIFO turn with no steering of the current turn. Routing depends only on residency: running enqueues, waiting wakes the same Agent, and an absent Activation cold-resumes a new one.
