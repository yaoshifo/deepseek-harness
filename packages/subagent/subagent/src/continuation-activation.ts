...
import type { SubagentSettlementDelivery } from './types.ts'
...
export class ContinuableActivationRegistry {
  ...
  constructor(
    ...
    this.setupContributions = options.setupContributions
    this.settlementDelivery = options.settlementDelivery ?? 'inbox'
    ...
    // lets a jobs-held watcher re-check residency while the Agent itself
    ...
    ctx.inject(['jobs'], (jobsCtx: Context) => {
      ...
      const disposeListener = jobs.onJobsChanged((owner) => {
        ...
        const activation = this.resident.get(owner.id)
  ...
  /** Classify one Inbox, owned-child, and live-job observation without reading Agent execution state. */
  ...
  private notifySettlement(activation: Activation, terminal: ActivationTerminal): void {
    // 'external' delivery: the deployment listens for `subagent/end` and
    // drives the parent's next turn itself, so the runtime's wake or queue
    // would only spend a model request the deployment never asked for. The
    // child's own Session and the `subagent/end` event remain the full
    // durable record, so nothing model-visible is lost.
