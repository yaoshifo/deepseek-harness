...
Lists the continuable children below the calling agent: `children` (default) shows direct children, `descendants` walks the whole tree in stable pre-order, annotating each entry with its durable direct-parent session id (`parent=<id> depth=<n>`) and depth. The `parent` annotation is the durable direct-parent session id and may name an ordinary session omitted from the output. For the calling agent, only depth-1 child entries are `send_message` candidates; deeper child entries are `interrupt_agent` candidates only. Status comes from the live Agent registry — `running` (active driver), `idle` (resident between turns, possibly waiting on agents it started), or `ready` (storage only and resumable rather than terminal); every non-empty render appends a one-line legend restating those meanings, because a bare `[ready]` tag read without the tool description at hand reads as "queued and still executing". One-shot children are intentionally absent because they cannot accept `send_message` (they stay in the service result for UI consumers), and unreadable candidates appear as diagnostics with positions in the descendants scope.
