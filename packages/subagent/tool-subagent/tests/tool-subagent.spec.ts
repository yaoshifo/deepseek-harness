...
describe('dsh-tool-subagent', () => {
  ...
  it('omits cwd by default and refuses a forced cwd at execution time', async () => {
  ...
  it('exposes cwd when enabled and passes it through to the provider request', async () => {
    const starts: SubagentStartRequest[] = []
    const ctx = await setup({ provider: 'mock', allowCwdOverride: true }, { onStart: (r) => { starts.push(r) } })
    const schema = ctx.tools.schemas().find(s => s.name === 'subagent')
    const props = (schema!.parameters as { properties?: Record<string, unknown> }).properties ?? {}
  ...
  it('treats an unknown (plugin-added) stop reason as an isError result', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'weird',
      capabilities: { agentOptions: false, outputSchema: false, depthLimit: false, toolFilter: false, persona: false, cwdOverride: false },
  ...
  it('passes persona/toolFilter/maxDepth config through to the start request', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'capture2',
      capabilities: { agentOptions: false, outputSchema: false, depthLimit: true, toolFilter: true, persona: true, cwdOverride: true },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
  ...
  it('a partial toolFilter (deny only) does not materialize an empty allow-list (deny-all trap)', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'capture3',
      capabilities: { agentOptions: false, outputSchema: false, depthLimit: false, toolFilter: true, persona: false, cwdOverride: false },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
...
describe('dsh-tool-subagent continuable background mode', () => {
  ...
  it('isolates a cancelled continuable preparation from a concurrent sibling', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'gated',
      capabilities: { agentOptions: false, outputSchema: true, depthLimit: true, toolFilter: true, persona: true, cwdOverride: true },
      inheritsParentContext: false,
