...
- **Zod 生成器只支持有意限定的子集**——条件类型、映射类型、索引访问类型等计算构造在 schema 内任意位置都会失败，枚举声明也会失败，因为模型只保留其成员初始化器文本；泛型声明会生成为 schema 工厂函数，但 schema 导出根必须是具体声明。
- **不支持泛型 Remote 方法**——带类型参数的 Remote 方法会使构建失败；wire 调用描述符为每个方法固定一套 schema，也不携带类型参数。
- **Remote boundary 类型必须从公开的非根 type 子路径导出**——仅通过根导出 `.` 导出的类型会使构建直接失败，唯一例外是其包位于生成器硬编码的 `PUBLIC_REMOTE_TYPE_ROOTS` allowlist（当前只有 `@deepseek-ai/dsh-util-values`）；新包要加入该 allowlist 只能修改生成器源码。
