/** Package-owned durable memory-index invariants. @module @deepseek-ai/dsh-memory/invariant */
...
/** The scope one recorded dsh-memory source belongs to; pre-scope (version 1) injections are project ones. */
function sourceScope(source: { scope?: unknown }): 'project' | 'global' {
  return source.scope === 'global' ? 'global' : 'project'
}

/**
 * Validate one durable dsh-memory index injection: exactly one text block
...
function validateInjection(history: readonly SessionEvent[], event: SessionEvent<'user/message'>, fail: InvariantFailure): void {
  ...
  if (event.data.content.length !== 1 || typeof text !== 'string') {
    fail('dsh-memory messages must contain exactly one text block')
  }
  if (!text.startsWith('<system-reminder>\n') || !text.endsWith('\n</system-reminder>')) {
    fail('dsh-memory message must be framed by the plugin-owned system-reminder')
  }
  ...
  if (source.kind !== 'dsh-memory') fail('dsh-memory source must retain package ownership')
  const scope = sourceScope(source)
  if ((source as { version: number | undefined }).version !== 2) {
    fail('dsh-memory source must carry version 2')
  }
  if (scope === 'project') {
    if (typeof source.project !== 'string'
      || source.project.length === 0
      || !source.project.startsWith('-')) {
      fail('dsh-memory project-scope source must carry a project slug')
    }
  } else if (source.project !== undefined) {
    fail('dsh-memory global-scope source must not carry a project slug')
  }
  if (typeof source.digest !== 'string' || !DIGEST.test(source.digest)) {
    fail('dsh-memory source must carry a SHA-1 digest')
  }
  const earlier = history.filter(prior =>
    prior.type === 'user/message' && prior.data.source.kind === 'dsh-memory'
    && sourceScope(prior.data.source) === scope)
  if (earlier.length > 0) {
    fail(`dsh-memory injects its ${scope} index at most once per session`)
  }
}
...
function validateSession(session: Session, fail: InvariantFailure): void {
  // oxlint-disable-next-line typescript/no-deprecated -- Invariant inspects the log; migration deferred.
...
/**
 * Register the dsh-memory invariant companion.
