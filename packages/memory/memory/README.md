...
Within one session, memory tools execute exclusively — the tool runtime runs a tool that declares no concurrency safety alone, in submission order — so same-message bursts of memory calls serialize. Across sessions and processes nothing coordinates writers: concurrent `memory_index` calls on one directory can lose each other's pointer lines (each call rewrites the whole index from its own read), and Claude Code sessions writing the same directory are equally uncoordinated. The global directory's writer population is every dsh session on the machine. Writes are atomic (temp file plus rename) and create the directory lazily. A file deleted between `memory_list`'s directory read and its per-file stat is skipped from the listing. A transient read failure at session start skips that injection; the memory tools still fail loud with the real error when called. A `claudeHome` whose directories do not exist yet is not an error: the section still renders, a scope without `MEMORY.md` injects nothing, and the first write creates the directory lazily.
...
The five generated schemas ([`memory_list` / `memory_read` / `memory_write` / `memory_delete` / `memory_index`](../../../docs/tool-catalog.md#deepseek-aidsh-memory)); each additionally carries the `scope` parameter and a description sentence pointing global reads and writes at the cross-project directory (absent only when the scope is disabled). Results: `memory_list` renders `name (bytes)` lines or `No memory directory yet.`; `memory_read` returns the file verbatim (a miss retries the `.md` suffix both ways); `memory_write` renders `Wrote <lines> lines (<bytes>B) to <name>[ + provenance frontmatter][. <index warning>]` and, when the written topic file has no pointer line in MEMORY.md, appends `No pointer line for <name> in MEMORY.md yet — add one with memory_index (action upsert, name, title, hook) so future sessions recall it.` (the structured result carries `indexed`, `true` for `MEMORY.md` itself); `memory_delete` renders `Deleted.` or `No such file.`; `memory_index` renders `Upserted index pointer for <name>; index now <lines> lines (<bytes>B).`, `Removed index pointer for <name>; …`, or `No index pointer for <name>.`. Stable failures: `Error: invalid memory name: …` (single-segment validation; the index also rejects `MEMORY.md` as its own key), `Error: memory not found: <name>`, `Error: memory_index upsert requires a non-empty title|hook` / `… must be a single line`, `Error: memory tools require a session working directory`, `Error: memory tools require an owning agent session`, `Error: memory tools are unavailable for subagent sessions`, `Error: global memory scope is not enabled in this deployment`.
...
- **Fork-local by decision (2026-09-06)** — upstream declined memory as a DSH capability (the 2026-07-31 third-party-memory-mcp-examples note: no vendor-specific plugin, no universal memory service — MCP overlay examples only), so this package is not awaiting upstreaming and is not proposed there; upstream absorptions treat `packages/memory/**` as fork-owned content like feishu-bridge. The durable kind `'dsh-memory'` is committed to released session logs and cannot be renamed under the adjacent-migration policy — if upstream later ships its own memory capability, adopt the upstream surface and replay the fork increments instead of renaming this one. Storage stays Claude Code-shaped on purpose, so a deployment can swap this plugin for an MCP memory server with no DSH-side data entanglement.
...
- **Cross-session index updates uncoordinated** — within one session the tool runtime serializes memory tools (exclusive execution, submission order); across sessions and processes a concurrent `memory_index` pair can silently lose a pointer line, and Claude Code writers are uncoordinated in the same way (CC 2.1.228 ships `memory_list`/`memory_read`/`memory_write` only — its index maintenance is prompt-discipline whole-file rewrites with no locking; its team-memory layer resolves conflicts by server-side overwrite plus user warnings). A 2,650-session log scan of one deployment found zero real cross-session overlaps — the only same-window writers were stall-resume double executions writing identical content — so no locking ships. Revisit when a real cross-session overlap is observed, chatroom persona memory volume grows, or CLI and daemon sessions routinely share a workdir; the ready fix wraps `updateMemoryIndex` (and a full-index `memory_write`) in `withFileLock` from `@deepseek-ai/dsh-atomic-write`.
