...
Claude Code memory compatibility plus an optional dsh-only global scope: dsh sessions read and write the same per-project memory directory Claude Code owns (`~/.claude/projects/<slug>/memory/`), so memories accumulated in Claude Code are recalled by dsh and memories written by dsh appear in Claude Code's next session. When `global` is configured, a cross-project memory directory (`~/.claude/memory/`) is shared by every dsh session regardless of project; Claude Code does not read or write it. The plugin introduces no storage of its own — storage, format, slug encoding, and index discipline are locked to Claude Code's observed behavior (leaked system prompt cross-checked against on-disk layouts).

## What it contributes

Three model-visible surfaces, all gated to top-level POSIX-cwd sessions (subagents get none):

1. **Memory strategy section** (`ctx.systemPrompt.section`, order 110): the verbatim Claude Code `## Memory` prompt with the directory instantiated, adapted only where dsh names differ — `the Write tool` becomes the memory tools, and the index paragraph carries two dsh-only sentences (maintain pointers with `memory_index`; generic file tools are sandbox-denied in the memory directory). When the global scope is enabled, a `## Global memory` appendix follows with the scope decision rule (see below). Model-facing text lives in `MEMORY_PROMPT` and `GLOBAL_MEMORY_PROMPT` (`src/prompt.ts`); anchor tests pin the load-bearing sentences.
2. **Session-start index injection**: the first admitted step of each session folds the project's `MEMORY.md` (first `maxIndexLines` lines or `maxIndexBytes` bytes, whichever comes first) into durable context as a sourced `user/message` (`{ kind: 'claude-memory', version: 2, scope, project?, digest }`), framed by a plugin-owned `<system-reminder>` that declares recalled memories background context, not user instructions. With the global scope enabled, the global index is injected first under the same framing and its own budget. Injection happens at most once per scope per session log (resume and compaction do not re-inject; the model reads fresher state with `memory_read`). No `MEMORY.md` means no injection for that scope.
3. **Five tools** on `ctx.tools`, operating only inside a memory directory through `node:fs` on the host — never the swappable `ctx.fs` provider, so the shared directories stay machine-local in every deployment shape: `memory_list`, `memory_read`, `memory_write`, `memory_delete`, `memory_index`. With the global scope enabled every tool takes an optional `scope: 'project' | 'global'` parameter (default project); without it the parameter does not exist and a passed `scope: 'global'` fails loud.
...
| `global.maxIndexBytes` | — | Enables the global scope. Byte budget for the session-start global `MEMORY.md` read; must be a positive number when `global` carries any key. A budget smaller than the project one is the noise hard cap. |
| `global.maxIndexLines` | `200` | Line budget for the global read; the earlier limit wins. |

An absent (or empty) `global` key disables the global scope completely: no `## Global memory` appendix, no global injection, no `scope` tool parameter — the model-visible surfaces stay byte-identical to a global-less deployment.
...
A `# Memory` system-prompt section present on every request of a top-level session with a POSIX cwd. The text is the verbatim Claude Code memory strategy; the directory is instantiated per session. With the global scope enabled, a `## Global memory` appendix follows, carrying the scope decision rule, the fail-safe default, and the lazy promotion rule, with the global directory instantiated.
...
##### Verbatim global appendix (enabled deployments only)
...
The five generated schemas ([`memory_list` / `memory_read` / `memory_write` / `memory_delete` / `memory_index`](../../../docs/tool-catalog.md#deepseek-aidsh-tool-claude-memory)); with the global scope enabled, each additionally carries the `scope` parameter and a description sentence pointing global reads and writes at the cross-project directory. Results: `memory_list` renders `name (bytes)` lines or `No memory directory yet.`; `memory_read` returns the file verbatim (a miss retries the `.md` suffix both ways); `memory_write` renders `Wrote <lines> lines (<bytes>B) to <name>[ + provenance frontmatter][. <index warning>]`; `memory_delete` renders `Deleted.` or `No such file.`; `memory_index` renders `Upserted index pointer for <name>; index now <lines> lines (<bytes>B).`, `Removed index pointer for <name>; …`, or `No index pointer for <name>.`. Stable failures: `Error: invalid memory name: …` (single-segment validation; the index also rejects `MEMORY.md` as its own key), `Error: memory not found: <name>`, `Error: memory_index upsert requires a non-empty title|hook` / `… must be a single line`, `Error: memory tools require a session working directory`, `Error: memory tools require an owning agent session`, `Error: global memory scope is not enabled in this deployment`.
...
- **No scheduled project→global promotion** — deliberate: the write-time rule and lazy re-filing carry more context than a periodic scan, and unattended writes into globally injected content would bypass human gatekeeping. The upgrade path is an on-demand audit skill if misfiling proves common.
