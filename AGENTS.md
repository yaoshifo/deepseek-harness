...
packages/    @deepseek-ai/dsh-<pkg> workspaces at packages/<group>/<pkg>/
  ...
  acp/         automation-only ACP server + cc-connect bridge (fork-local, see its README)
...
`CLAUDE.md` symlinks `AGENTS.md` at root, `packages/`, and `examples/`; edit the real file. Keep each rule self-contained while linking high-level docs. Condense when clarity survives; raise a `verify-doc-budgets` ceiling when content genuinely needs more space.
