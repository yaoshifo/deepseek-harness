...
pnpm run test:docs      # quick documentation checks (doc-quick aggregate)
...
- **Fork policy (secondary development on `dev`):** skip `doc-sync` and the generated-document gates (`gen-*-catalog`, `gen-doc-graphs`, `gen-scoped-events`, `verify-translation-pairing`, `verify-type-equiv`); doc/source drift is accepted, no regeneration. This overrides the doc-sync requirement above and in linked skills. Exception: [dsh-sync-upstream](.agents/skills/dsh-sync-upstream/SKILL.md) step 4 still runs its six generators after upstream merges — `packages/extensions/tool-cordis/src/api-catalog.ts` is generated runtime source nothing else catches. Follow the [fork secondary-development principles](.agents/notes/implemented/process/2026-08-29-fork-secondary-development-principles.md) for sync cadence, change placement, toolchain parity, and proposing seam features upstream once stable.
- Never default to the full suite or repeat a passing check for commit or push; CI owns exhaustive coverage and the platform matrix. Rehearse locally only by explicit request, CI diagnosis, or an irreducibly repository-wide change.
