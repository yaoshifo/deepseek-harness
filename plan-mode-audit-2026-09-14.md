...
| 本文条目 | 09-12 报告 | 关系 |
| --- | --- | --- |
| **F1 回放门禁已红（实测）** | 无 | **新增**。09-12 报告只在方案里写「改完刷新工具描述相关快照」，未发现刷新漏了 7 份、门禁当前即为红 |
...
- 失效清单（7 份，逐字节核对；全库 217 份 `expected.*` 中 37 份已带 fork 文案、2 份仍是旧措辞 `revise and present again`）：
  ...
  | `snapshots/acp/dsh-memory` | 有 `details`，缺 inlined-details 拒绝句（半过期） | acp lane（该簇另有既存 key 失败） |
...
**建议**：刷新受影响 lane（网页 + 会话 + acp），两份 pwsh 样本手工对齐；刷新 diff 人工过一遍确认无其他漂移；把「改了 `EXIT_DESCRIPTION` 要刷哪些 lane」写进 owner 清单。**注意顺序**：09-12 报告 A 组还要再改一次工具描述文案（它的 #6），必须「先改文案、后刷新」，只刷一次。
...
**建议**：两处各先写红测试（stub 渲染「先写盘再挂起」；失败后再呈现同内容），再分别改判断顺序与哈希记录时机；与 09-12 报告 B 组同批做（它已含 #4）。
...
- **Wave 2（串行，必须最后）**：**F1 的样本刷新与复验**。硬约束：G1 里所有模型可见文案（工具描述、拒绝文案）改完才刷，只刷一次；受影响 lane = 网页 + 会话 + acp，两份 pwsh 样本手工对齐；刷新后 `git diff snapshots/` 人工过一遍。
