...
| `tsconfig.base.json` | Shared compilerOptions and the source `paths` map. Also the resolution map for Vite's native `resolve.tsconfigPaths` in the vitest configs and for tsx's get-tsconfig consumers: it has no `include`, so its `paths` apply to any importer through `extends`. | No |
