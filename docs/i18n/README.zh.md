...
pre-commit 与 pre-merge-commit 钩子会对已记录配对中每一侧被暂存的 Markdown 运行这条限定范围检查——`foo.md` 和 `foo.zh.md`，而不只是 `foo.i18n.yaml`——因此只改一侧而不重新记录的提交在落地之前就会失败。没有一致性记录的被暂存 Markdown 不在钩子范围内；全语料的配对完整性仍由 `doc-sync` 与 CI 负责。
