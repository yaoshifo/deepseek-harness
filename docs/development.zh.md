...
| `tsconfig.base.json` | 共享 compilerOptions 与源码 `paths` 映射。同时充当 vitest 配置里 Vite 原生 `resolve.tsconfigPaths` 与 tsx 等 get-tsconfig 消费方的解析映射：它没有 `include`，经 `extends` 其 `paths` 适用于任何 importer。 | 否 |
