...
`ApprovalOutcome` is closed and fail-closed. `allowed-once` grants only the asked-about action; `allowed-always` additionally records an in-memory standing grant for the (agent, tool) pair, so later asks decide without dispatching; callers deny on `rejected`, `cancelled`, and `unavailable`. A missing, non-owning, throwing, or non-conforming answerer becomes `unavailable` rather than opening the gate.
...
Answerers may return a bare outcome or an `ApprovalAnswer` carrying a bounded human note; the service normalizes both and settles every ask into one `ApprovalResult`.
...
`ApprovalRequest` identifies the agent and tool action closely enough to route and audit the question. It deliberately omits tool arguments from the audit: an answerer attaches the prompt to the already-streamed tool call through `callId`, and `toolInput` carries only a bounded UI preview (the raw arguments already live in the paired `tool/call`).
