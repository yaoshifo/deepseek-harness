...
[English](feishu-bridge.md) | 中文

Feishu bridge 承载面向 bot 的对话运行时：每个配置项目一个 engine、平台适配器，以及兄弟插件缝。[chatroom 抽包 Agent Note](../../.agents/notes/implemented/architecture/2026-08-29-feishu-bridge-chatroom-extraction.zh.md) 负责服务与事件决策；本页记录 [`packages/acp/feishu-bridge/src/bridge-service.ts`](../../packages/acp/feishu-bridge/src/bridge-service.ts) 的 Cordis 面。

`ctx.feishuBridge` 服务暴露 live 项目注册表、caller 路由与 `feishuBridge/*` 分发面。兄弟插件（chatroom 包 `@deepseek-ai/dsh-feishu-bridge-chatroom`）经包的 `./exports` 入口消费该服务，通过应答分发事件而非挂载 engine 钩子来扩展引擎行为。
